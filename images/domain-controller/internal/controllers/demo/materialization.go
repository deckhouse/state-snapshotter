...
func demoManifestCaptureTargetsFromVCR(
	...
	merged := append([]ssv1alpha1.ManifestTarget(nil), base...)
	if vcrName != "" {
		...
		if getErr == nil {
			...
			owned := manifestTargetsFromVolumeTargets(targets)
			merged = appendOwnedPVCManifestTargets(base, owned, nil, namespace)
		}
	}
	return merged, nil
}
