...
// by its GVK (the namespaced root Snapshot, or any registered domain snapshot CR kind). It compiles
// that node and everything below it, so the restore endpoint can return apply-ready manifests for a
// single subtree, not only the whole namespace.
