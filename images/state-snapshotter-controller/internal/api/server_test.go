...
var _ = Describe("NewServer", func() {
	...
	Describe("mTLS is mandatory", func() {
		It("should fail to create server when CA is not provided", func() {
			server := NewServer(":8443", fakeClient, fakeClient, log, nil, nil, nil, serverCertFile, serverKeyFile, nil, nil)
	...
	Describe("mTLS mode", func() {
		It("should create server in mTLS mode when CA is provided", func() {
			...
			Expect(err).NotTo(HaveOccurred())
			server := NewServer(":8443", fakeClient, fakeClient, log, nil, nil, nil, serverCertFile, serverKeyFile, caCertBytes, []string{"system:kube-apiserver"})
		...
		It("should fail to create server when CA is invalid", func() {
			...
			server := NewServer(":8443", fakeClient, fakeClient, log, nil, nil, nil, serverCertFile, serverKeyFile, invalidCA, []string{"system:kube-apiserver"})
			Expect(server).To(BeNil())
		})
