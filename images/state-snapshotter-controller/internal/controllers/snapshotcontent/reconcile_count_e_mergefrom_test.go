...
//
//  1. CLOBBER-SAFETY: a sibling status field written by the snapshot reconciler (residualVolumeCapture) AFTER
...
func TestConditionsMergeFromPatchPreservesWriterStatusFields(t *testing.T) {
	...
	if err := unstructured.SetNestedField(writer.Object, storagev1alpha1.ResidualVolumeCapturePhaseComplete, "status", "residualVolumeCapture", "phase"); err != nil {
	...
	phase, _, _ := unstructured.NestedString(fresh.Object, "status", "residualVolumeCapture", "phase")
	if phase != storagev1alpha1.ResidualVolumeCapturePhaseComplete {
		t.Fatalf("writer field residualVolumeCapture.phase clobbered: got %q, want %q", phase, storagev1alpha1.ResidualVolumeCapturePhaseComplete)
	}
