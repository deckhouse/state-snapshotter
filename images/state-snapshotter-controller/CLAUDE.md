...
`KUBEBUILDER_ASSETS` points at the dir containing `etcd`, `kube-apiserver`, `kubectl` (e.g. `setup-envtest use 1.33.0 --arch amd64`). If assets are missing, BeforeSuite fails with a clear message (no silent skip). See the internal e2e testing strategy (root `CLAUDE.md` → "Docs: SSOT & boundaries").
...
- **CRD status (MUST):** our CRD statuses use **conditions only** (no custom phase/status strings). First-class resources: unified Snapshot / SnapshotContent (unstructured), ManifestCheckpoint / MCR — use `pkg/snapshot` condition & finalizer constants; follow the internal controller-pattern doc.
...
- Preconditions for progressing unified Snapshot / SnapshotContent reconcile (domain handled, refs valid, optional CRDs present) are defined in the internal system spec and controller-pattern doc, plus the long-form ADR.
...
- Ambiguous/invalid input: same rule — no partial inconsistent state. Use `pkg/snapshot` constants for condition types/reasons and finalizers; the internal spec + architecture docs are the source of truth for classification.
...
- Execution policy (where, redeploy, local/remote, e2e suite): root `CLAUDE.md` + the internal e2e testing strategy.
- Levels: Unit (logic), Envtest (controllers), E2E (real cluster). Unit/envtest run locally under `test/`; cluster smoke (`./hack/test-smoke.sh`) when applicable; remote-only suites only if documented in the internal strategy doc or CI.
- Scenario groups: unified snapshots, manifest/MCR, future DSC/registry, webhooks. Adding tests: check the internal strategy doc, avoid duplicates, add to the right group. Removing: only if the scenario is gone/replaced, then update that doc.
- **CEL / apiserver-validation MUST have an integration test (MUST):** a CEL rule (`XValidation` → `x-kubernetes-validations`) or structural CRD constraint (`required`/`minItems`/`enum`) is enforced by the kube-apiserver, and a fake-client unit test bypasses it (proves nothing). Any new/changed such rule requires an envtest under `test/integration/` asserting apiserver reject-on-violation + accept-on-valid. Keep the SDK/controller-side guard unit-tested too (complementary). See `.cursor/rules/cel-requires-integration-test.mdc`.
...
- E2E strategy must support: automated remote validation, environment prep (MinIO/S3, controller ns, storage class), and generation of ordered demo YAML artifacts. Do not remove diagnostic/inspection scenarios without an explicit replacement. Debug scenarios may live outside the main regression suite (optional/diagnostic in the strategy doc).
