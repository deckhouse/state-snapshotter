...
`KUBEBUILDER_ASSETS` points at the dir containing `etcd`, `kube-apiserver`, `kubectl` (e.g. `setup-envtest use 1.33.0 --arch amd64`). If assets are missing, BeforeSuite fails with a clear message (no silent skip). See `testing/e2e-testing-strategy.md` in the internal docs repo (root `CLAUDE.md` → "Docs: SSOT & boundaries").
...
- **CRD status (MUST):** our CRD statuses use **conditions only** (no custom phase/status strings). First-class resources: unified Snapshot / SnapshotContent (unstructured), ManifestCheckpoint / MCR — use `pkg/snapshot` condition & finalizer constants; follow `architecture/controller-pattern.md` (internal docs repo).
...
- Preconditions for progressing unified Snapshot / SnapshotContent reconcile (domain handled, refs valid, optional CRDs present) are defined in the internal docs repo — `spec/system-spec.md` and `architecture/controller-pattern.md` — plus the long-form ADR in the ADR repo (`dkp/storage/state-snapshotter/old2/`).
...
- Execution policy (where, redeploy, local/remote, e2e suite): root `CLAUDE.md` + `testing/e2e-testing-strategy.md` (internal docs repo).
- Levels: Unit (logic), Envtest (controllers), E2E (real cluster). Unit/envtest run locally under `test/`; cluster smoke (`./hack/test-smoke.sh`) when applicable; remote-only suites only if documented in the strategy doc or CI.
