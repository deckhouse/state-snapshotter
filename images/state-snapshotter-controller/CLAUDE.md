...
`KUBEBUILDER_ASSETS` points at the dir containing `etcd`, `kube-apiserver`, `kubectl` (e.g. `setup-envtest use 1.33.0 --arch amd64`). If assets are missing, BeforeSuite fails with a clear message (no silent skip). See `docs/state-snapshotter-rework/testing/e2e-testing-strategy.md`.
...
- **CRD status (MUST):** our CRD statuses use **conditions only** (no custom phase/status strings). First-class resources: unified Snapshot / SnapshotContent (unstructured), ManifestCheckpoint / MCR — use `pkg/snapshot` condition & finalizer constants; follow `docs/architecture/controller-pattern.md`.
...
- Preconditions for progressing unified Snapshot / SnapshotContent reconcile (domain handled, refs valid, optional CRDs present) are defined in `docs/state-snapshotter-rework/spec/system-spec.md`, the ADR under `snapshot-rework/`, and `docs/architecture/controller-pattern.md`.
...
- Execution policy (where, redeploy, local/remote, e2e suite): root `CLAUDE.md` + `docs/state-snapshotter-rework/testing/e2e-testing-strategy.md`.
