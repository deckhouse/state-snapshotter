...
// Kubernetes client rate-limit override env vars. Unset/empty keeps the built-in defaults; the values are
// read ONCE at process start (rest.Config.QPS/Burst are consumed when clients are constructed), so changing
// them requires a pod restart / rollout restart, not a hot reload.
const (
	EnvKubeQPS      = "STATE_SNAPSHOTTER_KUBE_QPS"
	EnvKubeBurst    = "STATE_SNAPSHOTTER_KUBE_BURST"
	EnvCaptureQPS   = "STATE_SNAPSHOTTER_CAPTURE_QPS"
	EnvCaptureBurst = "STATE_SNAPSHOTTER_CAPTURE_BURST"
)
