...
// T4 + eligibility-loss: Serial so CSD set does not collide with smoke/hot-add on the same RegistrationTest snapshot kind.
var _ = Describe("Integration: unified runtime RBAC and eligibility", Serial, func() {
	...
	It("does not add watches while CSD is Accepted but RBACReady is unset (T4)", func() {
		...
		Eventually(func(g Gomega) {
			...
			for _, p := range st.EligibleFromCSD {
				g.Expect(p.Snapshot).NotTo(Equal(regSnapGVK),
					"Accepted without RBACReady must not contribute to eligible-from-CSD layer")
			}
		}).WithTimeout(30 * time.Second).WithPolling(200 * time.Millisecond).Should(Succeed())
	})
	...
	It("drops resolved pair when RBACReady goes false but keeps monotonic active key", func() {
		...
		Eventually(func(g Gomega) {
			...
			for _, gvk := range st.ResolvedSnapshotGVKs {
				g.Expect(gvk).NotTo(Equal(regSnapGVK), "resolved should drop RegistrationTestSnapshot when CSD no longer watch-eligible")
			}
		}).WithTimeout(30 * time.Second).WithPolling(200 * time.Millisecond).Should(Succeed())
