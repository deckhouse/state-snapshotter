...
// Regression for ChildrenSnapshotReady publication ordering in demo controllers (INV-DOMAIN-GATE): a leaf demo
// disk snapshot must publish ChildrenSnapshotReady=True only after its own domain request (the MCR) has been
// planned. The previous code published ChildrenSnapshotReady=True immediately after source validation, before the
// SnapshotContent and MCR existed - so there was a multi-second window where ChildrenSnapshotReady=True but no
// request and no checkpoint had been planned yet. The invariant below would fail on that old ordering.
var _ = Describe("Integration: demo disk ChildrenSnapshotReady publication ordering", func() {
	It("publishes ChildrenSnapshotReady=True only after the MCR request is planned", func() {
		...
		// Ordering invariant, checked continuously from creation: whenever ChildrenSnapshotReady=True is observed,
		...
		invariant := func(g Gomega) {
			...
			dr := meta.FindStatusCondition(d.Status.Conditions, snapshot.ConditionChildrenSnapshotReady)
			...
			g.Expect(contentName).NotTo(BeEmpty(),
				"ChildrenSnapshotReady=True but neither an MCR ref nor a bound SnapshotContent is present (request not planned yet)")
			...
			g.Expect(content.Status.ManifestCheckpointName).NotTo(BeEmpty(),
				"ChildrenSnapshotReady=True but the request was never planned (no MCR ref, no handed-off checkpoint)")
		}
		Consistently(invariant).WithTimeout(8 * time.Second).WithPolling(150 * time.Millisecond).Should(Succeed())

		// And the flow does reach ChildrenSnapshotReady=True for the current generation with the honest planning
