...
5. В обычном leaf/simple-parent пути контроллер объявляет все ноги захвата, а затем
   **барьер 1** (`MarkPlanned`, `phase=Planned`).
...
Это обычный поток. Subtree-gated агрегатор намеренно сначала замораживает/публикует детей через
`MarkPlanned`, а свою ногу `EnsureManifestCapture(base − exclude)` объявляет позже; см.
«Manifest-exclude для агрегаторов». Дальше в гайде — детали каждого шага.
...
Subtree-gated агрегатор — исключение: он вызывает `EnsureChildren`, затем `MarkPlanned`, чтобы
заморозить набор детей и дать core забайндить их content, после чего ждёт
`SubtreeManifestIdentities` и объявляет собственную manifest-ногу. `MarkPlanned` перед этой поздней
ногой — намеренный протокол, а не недопланированный leaf. Референс — POC-контроллер
...
Эти идентичности появляются только после bind и персистирования content детей, поэтому референсный
...
единственное документированное исключение из обычного правила «все Ensure до MarkPlanned».
