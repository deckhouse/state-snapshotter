...
- Пакет `images/state-snapshotter-controller/pkg/unifiedbootstrap/`: единственный built-in список `DefaultGraphRegistryBuiltInPairs()` (только core `Snapshot`→`SnapshotContent`), `ResolveAvailableUnifiedGVKPairs(mapper, pairs, log)`. Отдельного, более широкого «runtime bootstrap» списка нет: доменные типы попадают в discovery/watch **только** через eligible CSD.
...
| R3 | **Runtime (без рестарта pod):** подписка по формуле `Accepted`+`AccessGranted`+поколения; `Ready` не в предикате; **отписка не гарантируется** | Нет обязательности watch на stale тип; новые eligible — без рестарта | ✅ *(additive + layered state + proof-тест + gauges/log stale↔resolved; symmetric unwatch — ⬜)* |
...
- [x] Статусы **`Accepted`**, производный **`Ready`** по ADR; **`AccessGranted`** не пишет контроллер (копия из объекта).
...
2. На parent snapshot: **`childrenSnapshotRefs`** — полный declarative planned graph; после discovery/create — `PlanningReady=True`.
...
4. **`Ready=True`** у parent snapshot — только mirror bound `SnapshotContent Ready`; parent content `Ready` агрегируется `SnapshotContentController` из собственного result + required child contents (child failed → parent content `Ready=False` / `ChildrenFailed`).
...
**Целевая архитектура vs код.** В модели PR5 / demo-domain — только **heterogeneous** flow ([`demo-domain-csd/README.md`](demo-domain-csd/README.md)). Ранний временный in-repo scaffold для PR2–PR3 **удалён**; тот же контракт доказывают **`snapshot_graph_integration_test.go`**, **PR5a/PR5b** и unit-тесты children-readiness / terminal child-Snapshot failure (`child_snapshot_terminal_failures_test.go`).
...
| **PR-8** | E2E local-thin | **`hack/demo-e2e.sh`** — manifest + VCR/VSC + retained + forced TTL GC | Rook/Ceph |
...
**Legacy phase map (superseded):** old P1–P5 in design §11 → use PR table above.

##### Implementation checklist (no code until PR-N)

Use as PR description / review gate. Check only items for that PR.

**PR-0 — Docs** ✅

- [x] `SnapshotContent` = logical node documented
- [x] §3.9.8 list vs ownership clarified
- [x] Root empty MCP + `manifestCheckpointName` documented
...
- [x] Envtest (`test/integration/n5_pr7_two_pvc_integration_test.go`): two-PVC subtree — child `dataRefs[]` covers `pvc-a`, root MCR only `pvc-b`
- [x] Pending VCR `spec.targets[]` counts as subtree coverage before `dataRefs[]` publish
- [x] Duplicate `pvcUID` in subtree → `DuplicateCoveredPVCUID`, no invalid root MCR
...
- [x] PR-8 cluster: **`hack/demo-e2e.sh`** — unified manifest + volume + retained (staged artifacts under `artifacts/<run-id>/`)

**PR-8 — E2E local-thin**

- [x] Unified cluster script **`hack/demo-e2e.sh`** (replaces separate PR-4/PR-8 as main path; local-thin only)
- [x] Run on cluster: MCP + `dataRefs[]` + aggregated + retained read (temporary `/snapshots/{name}/manifests`)

**PR-9 — Ops docs**

- [ ] Runbook: debug `dataRefs[]`, pending VCR, duplicate pvcUID
- [ ] `project-status.md`: N5 PR progress row

**Tests cross-reference:** design [`volume-node-dual-capture.md`](volume-node-dual-capture.md) §12 (D1–D9) map to PR-7/PR-8 themes.
...
2. ~~**Точечные integration-тесты**~~ ✅ — `unified_runtime_rbac_eligibility_test.go`: без AccessGranted нет записи в `EligibleFromCSD`; после снятия AccessGranted resolved теряет пару, monotonic active сохраняет ключ.
