...
- только: (1) опционально fail-fast валидирует, что minimum invariants соблюдены;
  (2) ставит internal condition (`PlanningReady=True`); (3) публикует planning barrier.
...
> **NB (исторически):** раньше это condition называлось `ChildrenSnapshotReady` и было
> misleading — фактически он означает не «children ready», а **«node planning completed»**.
> Сначала это спрятали за точным intent `MarkPlanningReady` (Р17, rename верба был архитектурно
> корректным, не косметическим), а затем переименовали и само условие в `PlanningReady` —
> теперь имя intent и condition совпадают.
...
>    `PatchCondition("PlanningReady", metav1.ConditionTrue)`, `PatchStatus(...)` — API leaking
...
- **Барьер `PlanningReady`** (`api/storage/v1alpha1/conditions.go`) — SDK выставляет его
  через intent `MarkPlanningReady` (Р17; бывш. misleading-имя `ChildrenSnapshotReady` переименовано в `PlanningReady`).
...
- Поля домена: `manifestCaptureRequestName`, `volumeCaptureRequestName`, `childrenSnapshotRefs`,
  условие `PlanningReady`, ранний `Ready=False` (валидация).
...
2. **Никаких raw strings / raw `metav1.Condition` на границе.** Плохо:
   `SetCondition("PlanningReady", metav1.ConditionTrue)` / `PatchCondition(..., cond)` —
...
**4) Переименование: `MarkChildrenReady` → `MarkPlanningReady`.**
...
condition `PlanningReady=True` (изначально это было misleading-имя `ChildrenSnapshotReady`,
спрятанное за точным intent; позже переименовано и само condition, §1/Р15).
...
- graph/children planning failure → `PlanningReady=False` (это `MarkPlanningFailed`, отдельный барьер).
...
`PlanningReady=False`, а не `Ready=False`). Source validation и artifact resolution — доменные.
...
- TOCTOU live-refresh (`!DataCaptured||!ManifestCaptured` → `APIReader.Get`), D4a
  `MergeFromWithOptimisticLock`, барьер `ConditionPlanningReady`, MCR/VCR ensure, ownerRef-хелперы,
...
1. **Publication `Ready=False` — обобщить в `MarkNotReady` — РЕШЕНО (Р30).** Демо публикует `Ready=False`
   ...
   `MarkPlanningFailed` остаётся отдельно (барьер `PlanningReady=False`). Детали — Р30.
...
type ReadinessFault interface {
    ...
    // publishes the outcome. MarkPlanningFailed is separate (it drives the PlanningReady barrier,
    // not Ready).
    MarkNotReady(ctx context.Context, t ReadinessFaultTarget, in NotReadySpec) error
}
...
- Children API (ревью 4, Р17, РЕШЕНО): **desired-set based, НЕ incremental** — домен отдаёт полный
  ...
  `MarkChildrenReady`→`MarkPlanningReady` (+`MarkPlanningFailed`); condition тоже переименовано `ChildrenSnapshotReady`→`PlanningReady`.
...
- Consistency-аудит (ревью 5, design vs фактический код — §5.1a):
  ...
  - Р30 (**РЕШЕНО**): publication `Ready=False` обобщена — узкий `MarkInvalidSource` заменён на
    ...
    artifact-missing). `MarkPlanningFailed` остаётся отдельно (барьер `PlanningReady=False`).
...
9. `snapshotsdk` (Р30): `MarkNotReady` — source invalid → `Ready=False` (no requeue); artifact-missing →
   `Ready=False` + `Requeue=true` intent; planning failed → `PlanningReady=False` (`MarkPlanningFailed`).
