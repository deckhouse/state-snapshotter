...
| `02-tree-ready` | форма дерева (root→VM→disk-vm; standalone disk как root child; covered disk не дублируется; ConfigMap в MCP, не child); happy-path `ManifestsReady/DataReady/ChildrenReady/Ready=True`; baseline mirror root `Snapshot.Ready == content.Ready` |
...
| `08-vsc-pending` | VSC `readyToUse=false` → `DataReady=False/DataCapturePending` (non-terminal) → root mirror |
| `09-vsc-recovery` | VSC `readyToUse=true` → recovery |
| `10-vsc-missing` | **destructive**: удаление VSC → `DataReady=False/ArtifactMissing`; sibling isolation; namespace чистится (восстановление невозможно) |
