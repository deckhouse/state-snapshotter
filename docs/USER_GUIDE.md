...
A `Snapshot` (`storage.deckhouse.io/v1alpha1`, short name `snap`) is a namespaced, **point-in-time capture of a namespace's desired state**. You create a `Snapshot` in a namespace, and the module captures the manifests of the user-owned objects in that namespace into an immutable, durably stored artifact that you can read back later or restore into another namespace.
...
| Property | Value |
|----------|-------|
| API group / version | `storage.deckhouse.io/v1alpha1` |
...
| Module machinery | the `state-snapshotter.deckhouse.io` group and `storage.deckhouse.io` request/transfer kinds (`VolumeCaptureRequest`, `VolumeRestoreRequest`, `DataExport`, `DataImport`) | internal execution objects |
