...
## Исключение объектов из снимка

`Snapshot` захватывает namespace целиком. Чтобы объект не попал в снимок, повесьте на него лейбл `state-snapshotter.deckhouse.io/exclude` **до** создания `Snapshot` — захват одноразовый, и лейбл, поставленный позже, уже созданный снимок не меняет:

```shell
d8 k -n myns label configmap scratch-cache state-snapshotter.deckhouse.io/exclude=""
```

Объект исключает само **наличие** лейбла; значение игнорируется (`""`, `true` — что угодно).

Исключение работает на любом уровне дерева снимка:

- **Обычный namespaced-объект** (`ConfigMap`, `Secret`, ...) выпадает из захваченных манифестов namespace.
- **Доменный ресурс** (например, `VirtualMachine`) вообще не разворачивается в дочерний снимок — его поддерево не захватывается целиком.
- **Объект внутри доменного поддерева** (например, один `VirtualDisk` захваченной ВМ или сопутствующий `Secret`) выпадает один; узел-владелец захватывается как обычно.
- **`PersistentVolumeClaim`** не захватывается data-ногой, а пользовательский CSI `VolumeSnapshot` поверх исключённого PVC остаётся обычным CSI-снапшотом — модуль его не адоптирует.

Исключение **не наследуется**: исключение доменного ресурса убирает его снимочное поддерево, но объекты, которые остальные ноги захвата видят сами по себе, — его `PersistentVolumeClaim`, сопутствующие объекты — продолжают захватываться, пока лейбл не стоит на них самих.

Исключённый объект **фиксируется, а не пропадает молча**: исключивший узел перечисляет его в `status.captureState.domainSpecificController.excludedRefs` (точные `apiVersion`/`kind`/`name`), а привязанный `SnapshotContent` агрегирует исключения всего дерева в `status.excludedRefs`. По этим полям можно убедиться, что исключение сработало.

> Как и встроенные исключения выше, лейбл может только **сузить** захват. Include-списка на снимок нет: захват — это всегда «namespace минус исключения». Чтобы захватывать разные подмножества, используйте разные namespace.
