...
- **MUST:** каждый CSD-registered **`XxxxSnapshot`** в parent-owned graph (кроме root **`Snapshot`**) имеет обязательный **`spec.sourceRef`** `{apiVersion, kind, name}` на namespace-local source object, который этот snapshot materializes. Положение snapshot в дереве публикует parent через **`status.childrenSnapshotRefs`**; Kubernetes **`ownerReference`** child → parent используется только для lifecycle/back-reference/requeue и не заменяет refs. **`Snapshot`** не имеет `sourceRef`: его source неявно равен **`metadata.namespace`**.
