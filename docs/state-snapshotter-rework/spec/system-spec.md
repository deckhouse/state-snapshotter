...
| **Graph registry state** | Пары `XxxxSnapshot` / `XxxxSnapshotContent`, доступные для `NamespaceSnapshot` discovery и generic graph traversal | Встроенный минимум только **`NamespaceSnapshot` → `NamespaceSnapshotContent`** + eligible DSC + `RESTMapper`; demo VM/Disk **не** активируются по умолчанию |
...
- **Dedicated controllers startup:** `NamespaceSnapshot`, `NamespaceSnapshotContent`, `DemoVirtualDiskSnapshot`, `DemoVirtualMachineSnapshot` controllers стартуют независимо от `STATE_SNAPSHOTTER_UNIFIED_ENABLED`. DSC управляет не фактом запуска этих controllers, а тем, входит ли domain kind в **graph registry** и `NamespaceSnapshot` discovery.
- **Target common content model (migration stage 1):** existing `storage.deckhouse.io/SnapshotContent` is the target common, cluster-scoped content carrier. It exposes `status.manifestCheckpointName`, `status.childrenSnapshotContentRefs`, and optional `status.dataRef`. Current runtime still uses dedicated `NamespaceSnapshotContent` / demo `*SnapshotContent`; DSC `contentCRDName` remains legacy v1alpha1 compatibility and is not removed in this stage.
...
- **N2b — форма графа в статусе (PR1+):** опциональные поля **`status.childrenSnapshotRefs`** на **`NamespaceSnapshot`** (элементы JSON: обязательные **`apiVersion`**, **`kind`**, **`name`**) и **`status.childrenSnapshotContentRefs`** на **`NamespaceSnapshotContent`** (элемент: **`name`**). В Go — **`NamespaceSnapshotChildRef`** / **`NamespaceSnapshotContentChildRef`**. Generic **не** резолвит дочерний snapshot «по имени» через перебор registry: **`apiVersion`/`kind`** задают объект однозначно. Для domain snapshot API parent задается обычным **`parentSnapshotRef`** (`apiVersion/kind/name`, namespace-local), без special-case поля "root NamespaceSnapshot". Семантика заполнения и orchestration — **§2.4.2** плана и design **§11**. **Инварианты дерева snapshot-run, merge и dedup** — **§3**; дизайн-пакет — [`design/demo-domain-dsc/README.md`](../design/demo-domain-dsc/README.md), [`design/demo-domain-dsc/08-universal-snapshot-tree-model.md`](../design/demo-domain-dsc/08-universal-snapshot-tree-model.md).
