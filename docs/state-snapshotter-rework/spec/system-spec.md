...
- **MUST (INV-COND4, mirror):** `Snapshot.Ready := mirror(SnapshotContent.Ready)` (status/reason/message копируются). Единственное исключение — bridge для терминального child-Snapshot capture failure до появления bound content (`Ready=False/ChildrenFailed`), который дерево контента ещё не может выразить.
