...
- **R2 phase 1 ✅:** reconciler в manager (`internal/controllers/csd/controller.go`): resolve маппинга по CRD, `Accepted` / производный `Ready`, `KindConflict` и `InvalidSpec` (в т.ч. namespace-scoped content); `RBACReady` пишет только hook.
...
- **Цель (ядро):** регистрация типов через CSD + **RBACReady** + активация watch без рестарта для новых eligible типов — реализовано для additive-пути; симметричное снятие watch — нет.
...
- **MUST:** root **`Snapshot`** treats CSD `priority` as layers/waves, not just item ordering. Higher numeric priority is planned first. A whole priority layer is processed together: all mappings in that layer are listed, all required child snapshots are created/ensured, and then traversal stops until every planned/existing snapshot in that layer has `DomainReady=True` (gen-gated). Lower-priority mappings **MUST NOT** be planned before coverage from the completed higher-priority layer is visible through the snapshot tree. If a child snapshot in the current layer reaches terminal failure (`DomainReady=False/GraphPlanningFailed` or terminal `Ready=False`), root **MUST** set an explicit `DomainReady=False` condition/error and stop lower-priority traversal instead of waiting forever. Retry remains possible after the child state is fixed.
...
- **MUST:** non-root / domain **`SnapshotContent` MUST NOT** be **data-only**: MCP **MUST** include at least the manifest of the snapshot's **source object** (identified by the generic source identity annotation, §3.2); if the node owns PVCs, MCP **MUST** also include sanitized PVC manifests for those PVCs; **`dataRefs[]`** holds the matching VSC bindings.
...
Публичная модель — **4 condition'а**: `DomainReady` (gate/барьер, §3.8), `RequestsReady`, `ChildrenReady`, `Ready`. `SnapshotContent` несёт `RequestsReady` / `ChildrenReady` / `Ready`; `Snapshot` несёт `DomainReady` (своё) и зеркалит `Ready` (§3.8).

- **MUST:** агрегат вычисляется ровно на `SnapshotContent` (**INV-COND2**, единственный агрегатор):

```text
SnapshotContent.RequestsReady = MCP Ready ∧ (∀ required dataRefs[].artifact Ready)
SnapshotContent.ChildrenReady = (∀ required child SnapshotContent Ready=True)   // leaf без детей → True
SnapshotContent.Ready         = RequestsReady ∧ ChildrenReady
```

- **MUST:** **`RequestsReady`** покрывает «свои requests узла»: ≤1 MCR + ≤1 VCR (**INV-COND5**). **`MCP Ready`** means **`status.manifestCheckpointName`** references an existing MCP with **`Ready=True`** (including **empty archive**, 0 objects). Applies to **aggregator-only** root: MCP may be **empty by content**, но **`manifestCheckpointName` MUST be present** и указывать на **Ready** MCP. **`SnapshotContentController` MUST** validate **`manifestCheckpointName`** одинаково для любого узла. **MUST NOT** вводить специальную ветку «root без MCP» в v1.
- **MUST:** manifest-only nodes treat an **empty** `dataRefs[]` as satisfying the data leg (**INV-COND6**); nodes that own PVCs **MUST** remain `RequestsReady=False` until **all** required bindings are Ready.
- **MUST:** validator order остаётся manifest → data (all refs) → children.
- **MUST (приоритет reason у `Ready`):** при нескольких упавших ногах несётся один reason в строгом порядке `RequestsFailed > ChildrenFailed > RequestsPending > ChildrenPending > Completed` (терминальные провалы первыми; свой узел перед детьми при равной тяжести).
- **MUST (INV-COND1, gate-импликация):** `RequestsReady=True ⇒ DomainReady=True`; requests не появляются без планирования. Это держит `Snapshot.Ready == Content.Ready` при том, что `DomainReady` **не** входит в формулу `Ready`.
...
- **MUST (progress-aware):** `Ready=False` **MUST NOT** be mute/generic. `RequestsReady=False` / `ChildrenReady=False` reason+message **MUST** explain what is awaited:
  - manifest leg: `ManifestCapturePending` (no `manifestCheckpointName` yet / MCP not Ready), `ManifestCheckpointFailed` (terminal);
  - data leg: `DataCapturePending` (published data artifacts not yet ready) with progress count `"<ready>/<total> ready"` and a capped pending list; `ArtifactMissing` / `DataArtifactInvalid` / `DataArtifactNotSupported` (terminal);
...
- **MUST NOT (API):** `dataRefs[]` **MUST NOT** be extended with a `snapshotRef` field in this slice (code-level TODO only). The durable artifact is the **VSC**; the original `VolumeSnapshot` is **not** guaranteed to outlive capture (it is GC-able once the VSC is retained). At restore time the VS → VSC relationship is therefore **not** assumed from a live VS: restore either **finds an existing `VolumeSnapshot`** that is bound to the retained VSC, or **recreates a `VolumeSnapshot` from the retained VSC** (pre-provisioned, `spec.source.volumeSnapshotContentName`). The PVC manifest + retained VSC are sufficient to drive this; a live VS is not required.
