...
- **Common content model (v0):** `storage.deckhouse.io/SnapshotContent` is the only active cluster-scoped content carrier. It exposes `status.manifestCheckpointName`, `status.childrenSnapshotContentRefs`, optional `status.dataRef`, and `status.conditions`. DSC mapping is `resourceCRDName -> snapshotCRDName`; the content side is fixed to common `SnapshotContent`. The common `SnapshotContentController` owns lifecycle/finalizers only for this GVK; `Ready`, MCP refs, child refs, and data refs are written by the snapshot-domain controllers.
...
- **MUST:** E5 subtree exclude строится как union object identities из MCP всех materialized descendant content-nodes, достижимых через **`childrenSnapshotContentRefs`**: typed **`SnapshotContent`** и registered dedicated **`SnapshotContent`**. Missing/NotReady descendant MCP — fail-closed pending; child manifests не записываются в parent MCP.
