...
- **Domain controller barrier (v0):** CSD-registered snapshot controllers (включая generic binder для бездетных snapshot) MUST set `status.conditions[type=DomainReady]=True` with `observedGeneration == metadata.generation` (gen-gated, **INV-COND3**) on their `XxxxSnapshot` after they have accepted/planned the snapshot and published required request/graph status fields (child refs, MCR/VCR). `GenericSnapshotBinderController` MUST wait for `DomainReady=True` at current generation before binding/creating `SnapshotContent`. `DomainReady` — единственное barrier-condition; любые прежние имена barrier-condition сняты и MUST NOT использоваться активными контроллерами или тестами.
