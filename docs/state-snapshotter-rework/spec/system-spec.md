...
Когда **`status.childrenSnapshotRefs`** **не пуст**, generic reconcile **`NamespaceSnapshot`** **MUST** выводить **`Ready`** родителя из **состояния каждого** дочернего snapshot, заданного ref с обязательными **`apiVersion`**, **`kind`**, **`name`**. Дочерний объект **MUST** читаться **одним Get** по этому GVK в namespace родителя; статус — **generic-способом** (**`unstructured`**: **`status.boundSnapshotContentName`**, condition **`Ready`**). **SnapshotGraphRegistry** на пути E6 **не** обязателен. Пробуждение родителя при изменении дочернего объекта — через **динамические watches** на зарегистрированные snapshot-GVK (см. **§0**) и map **child → parents** по совпадению ref **в пределах namespace дочернего объекта** (**§3.2**); **MUST NOT:** полагаться на внешний «kick» reconcile родителя от доменных контроллеров для корректности E6.

- **MUST:** при **NotFound** дочернего объекта — класс **ожидание** (ordering / ещё не создан). При ошибке Get (не NotFound) — **fail** операции агрегации на этом reconcile. Состояния «ambiguous ref» **нет** в контракте.
- **MUST:** классифицировать каждого resolved child как **успех** (bound + **`Ready=True`**), **ожидание**, или **терминальный провал** (**`Ready=False`** с причиной из согласованного набора terminal capture / fail-closed). Расширенные мотивы — [`design/demo-domain-dsc/07-ready-delete-matrix.md`](../design/demo-domain-dsc/07-ready-delete-matrix.md).
