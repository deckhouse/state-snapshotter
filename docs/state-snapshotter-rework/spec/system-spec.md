...
- **Common content model (v0):** `storage.deckhouse.io/SnapshotContent` is the only active cluster-scoped content carrier. It exposes `status.manifestCheckpointName`, `status.childrenSnapshotContentRefs`, optional `status.dataRef`, and `status.conditions`. DSC mapping is `resourceCRDName -> snapshotCRDName`; the content side is fixed to common `SnapshotContent`. `SnapshotContentController` is an aggregator/lifecycle controller, not a planner/executor: it does **not** create MCR/VCR/DataExport/VolumeSnapshot requests and does **not** decide what must be captured. It is the sole owner of `SnapshotContent.status`: it reads already-created manifest/data/child results through refs published on the bound snapshot status (for example `status.manifestCaptureRequestName` and `status.childrenSnapshotRefs`), writes MCP refs, child content refs, data refs, and `Ready`. Snapshot-domain controllers own planning/execution requests and their `XxxxSnapshot.status` fields, including `boundSnapshotContentName`, request names, `childrenSnapshotRefs`, and snapshot-level `Ready` mirrored from bound content. Request `Ready=True` means the final artifact is ready **and** handed off to `SnapshotContent` via ownerRef; requests are temporary execution objects, not sources of truth.
...
- **N2b — форма графа в статусе (PR1+):** опциональные поля **`status.childrenSnapshotRefs`** на **`NamespaceSnapshot`** (элементы JSON: обязательные **`apiVersion`**, **`kind`**, **`name`**) и **`status.childrenSnapshotContentRefs`** на **`SnapshotContent`** (элемент: **`name`**). В Go — **`NamespaceSnapshotChildRef`** / **`SnapshotContentChildRef`**. Generic **не** резолвит дочерний snapshot «по имени» через перебор registry: **`apiVersion`/`kind`** задают объект однозначно. Для domain snapshot API parent задается обычным **`parentSnapshotRef`** (`apiVersion/kind/name`, namespace-local), без special-case поля "root NamespaceSnapshot". Семантика заполнения и orchestration — **§2.4.2** плана и design **§11**. **Инварианты дерева snapshot-run, merge и dedup** — **§3**; дизайн-пакет — [`design/demo-domain-dsc/README.md`](../design/demo-domain-dsc/README.md), [`design/demo-domain-dsc/08-universal-snapshot-tree-model.md`](../design/demo-domain-dsc/08-universal-snapshot-tree-model.md).
...
- **MUST:** каждый DSC-registered **`XxxxSnapshot`** в parent-owned graph (кроме root **`NamespaceSnapshot`**) имеет обязательный **`spec.sourceRef`** `{apiVersion, kind, name}` на namespace-local source object, который этот snapshot materializes. **`spec.parentSnapshotRef`** описывает положение snapshot в дереве; **`spec.sourceRef`** описывает, какой domain object снимается. **`NamespaceSnapshot`** не имеет `sourceRef`: его source неявно равен **`metadata.namespace`**.
