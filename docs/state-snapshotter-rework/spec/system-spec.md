...
Design: [`design/status-propagation-and-visibility.md`](../design/status-propagation-and-visibility.md). One behavioural model, two phases (Phase 1 — progress-aware `Ready=False`; Phase 2 — damaged-artifact wake-up/revalidation). Pending and degradation share the **same** aggregation pipeline and reason taxonomy.
...
- **MUST (revalidation without watch):** if `SnapshotContentController` is woken while a published artifact ref no longer resolves (e.g. SnapshotContent was `Ready=True`, a later reconcile observes the referenced artifact missing/terminally failed), it **MUST** recompute `RequestsReady=False` (`ArtifactMissing` / `ManifestCheckpointFailed`) and therefore `Ready=False`, with the artifact kind/name in the message. The **wake-up** mechanism (artifact→content watches) is **Phase 2**; the recompute itself is part of this model and is unconditional on every reconcile.
