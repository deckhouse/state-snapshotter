...
| **Graph registry state** | Пары `XxxxSnapshot` / `SnapshotContent`, доступные для `Snapshot` discovery и generic graph traversal | Встроенный минимум только **`Snapshot` → `SnapshotContent`** + eligible DSC + `RESTMapper`; demo VM/Disk **не** активируются по умолчанию |
...
- **MUST:** legacy `snapshots/{name}/manifests` continues to work after root `Snapshot` deletion while the root ObjectKeeper and root `SnapshotContent` exist. Retained lookup is limited to `ObjectKeeper.spec.followObjectRef -> Snapshot` plus exactly one root `SnapshotContent.ownerRef -> ObjectKeeper`; zero matching content returns `404`, multiple matching contents returns `409 Conflict`. If the ObjectKeeper is deleting but content still exists, the endpoint may continue reading until GC removes the content.
