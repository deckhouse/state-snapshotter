...
- **N2b PR3:** агрегированный **Ready** parent при использовании **временного** synthetic required child: собственный persisted MCP **и** required child **`Ready=True`**; иначе **`ChildSnapshotPending`** или при терминальном провале child — **`ChildSnapshotFailed`** (`pkg/snapshot`); таблица и whitelist — **§11.1** design. Тот же **срок снятия scaffold**, что и для PR2.
