...
В текущем v0 есть один runtime mode: `SnapshotController`, common `SnapshotContentController`, DSC reconciler, graph registry, generic runtime syncer and dynamic hot-add watches always enabled.
...
curl --cacert "$TMP_CERT_DIR/ca.crt" \
  ...
  "$SERVER$API_PATH_BASE/snapshots/root-no-dsc/manifests"
```

## 1. CRD установлены

```shell
kubectl get crd | grep -E 'snapshots|snapshotcontents|demovirtual|domainspecificsnapshotcontrollers|manifestcapture|manifestcheckpoints'
```
...
wait_snapshot_ready snapshot root-no-dsc 120
```
...
- `Snapshot/root-no-dsc` имеет `Ready=True Completed`;
...
```shell
kubectl -n "$NS" delete snapshot root-no-dsc --ignore-not-found --wait=false
...
- Без DSC `Snapshot` готов и не создаёт demo children.
