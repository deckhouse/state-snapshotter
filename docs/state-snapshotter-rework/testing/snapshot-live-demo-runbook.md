...
| RBAC chunks (граф) | `kubectl auth can-i get manifestcheckpointcontentchunks.state-snapshotter.deckhouse.io` | `yes` (после `templates/rbac-for-us.yaml`) |
...
| **C — volume-only** | Показ manifest + **один** том без child tree | отдельный ns (`snapshot-demo-volume`) | 1× **Bound** PVC + consumer Pod + ConfigMap (опционально) |
| **B — полный N5** | Child + residual root + 2 PVC | `demo-e2e-*` или артефакты e2e | 2× Bound PVC + child Snapshot; эталон — `hack/demo-e2e.sh` |

**Правило:** треки A и C — **разные namespace**. Snapshot создаём **только после** preflight (для C — обязательно).
...
kubectl auth can-i get manifestcheckpointcontentchunks.state-snapshotter.deckhouse.io | grep -q yes
...
- **Нет** `MISSING` на Chunk (иначе RBAC или битый MCP)
...
1. Трек B: `hack/demo-e2e.sh` + `DEMO_E2E_SKIP_CLEANUP=1` и артефакты `06-root-ready/graph/*.svg`.
2. Трек A: `snapshot-demo-manifest`, только ConfigMap.
3. Трек C: `snapshot-demo-volume`, preflight-скрипт из runbook, `local-thin` + bind pod.
4. RBAC: `get` на `manifestcheckpointcontentchunks`.
5. Слайд «не показываем»: restore, list chunk, VolumeSnapshot restore intent.
