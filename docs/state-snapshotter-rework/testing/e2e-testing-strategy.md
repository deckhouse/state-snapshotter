...
- **retained read** после удаления `NamespaceSnapshot` выполняется по имени retained `SnapshotContent`; старое имя удалённого `NamespaceSnapshot` больше не резолвится обратно в content после удаления snapshot;
...
| `namespacesnapshot_n1_boundary_test.go` | **Формальное закрытие N1:** **recovery** — после сброса `status` при существующем deterministic content снова `Bound`+`Ready`; короткая **стабильность** (Consistently) |
