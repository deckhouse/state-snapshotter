...
| `namespacesnapshot_synthetic_tree_test.go` | **Synthetic child tree** (**временный scaffold в коде, не целевая модель PR5**): child NS; graph refs; parent ждёт child; **`ChildSnapshotPending`** / **`SubtreeManifestCapturePending`** → **`Completed`** (**§3-E6:** единый generic путь **`PickParentReadyReasonE6`** по typed **`childrenSnapshotRefs`**); **§3-E5 reconcile-path:** root **MCR** (когда существует) не содержит **ConfigMap**, уже в child **MCP**; без **terminal `CapturePlanDrift`** на пути к parent **Ready**; **контракт каскада:** child NSC имеет `ownerRef`→parent NSC, child с `FinalizerParentProtect`; при delete parent NSC контроллер снимает parent-protect с child (без `Delete` child из этого reconciler) |
| `namespacesnapshot_e5_subtree_mcr_gate_integration_test.go` | **§3-E5:** пока у child **NSC** нет **`manifestCheckpointName`** или child **MCP** не **Ready**, root **MCR** отсутствует; после **Ready** child **MCP** root **MCR** появляется и не листит **ConfigMap** из child subtree (**Eventually** без «успели увидеть» за счёт явных ожиданий) |
| `namespacesnapshot_synthetic_child_failure_test.go` | **N2b PR3 + §3-E6:** искусственный **CapturePlanDrift** только на **MCR child** (лишний target в `spec.targets`); child терминально **`Ready=False`**; parent остаётся с валидным N2a-планом → **`ChildSnapshotFailed`** (приоритет над любыми pending), message с именем child + причиной child |

**Synthetic integration tests** (`namespacesnapshot_synthetic_*`): **временный scaffold в коде, не целевая модель PR5** (в целевом narrative demo-domain synthetic **не** фигурирует). После зелёного **merge-gate** на demo-domain flow — **удалить** вместе с веткой **`n2b-pr2-synthetic-tree`** по правилу в [`design/implementation-plan.md`](../design/implementation-plan.md) **§2.4.2** (*After merge-gate on demo-domain flow, synthetic scaffold must be removed…*).
...
**N2b:** дерево — дочерние NS/NSC, **childrenSnapshotRefs** / **childrenSnapshotContentRefs**, агрегированный **Ready** parent (**§11.1** design; нормативный минимум **§3.8** [`spec/system-spec.md`](../spec/system-spec.md)), **aggregated manifests download** на чтении; **PR4** — нормативный контракт HTTP/ошибок/обхода: [`spec/namespace-snapshot-aggregated-manifests-pr4.md`](../spec/namespace-snapshot-aggregated-manifests-pr4.md); поставка короткими PR — [`design/implementation-plan.md`](../design/implementation-plan.md) **§2.4.2**; **PR2** — `namespacesnapshot_synthetic_tree_test.go` (**временный scaffold в коде, не целевая модель PR5**; до merge-gate на demo flow); **PR3** — матрица parent reason (**`ChildSnapshotFailed`** > **`SubtreeManifestCapturePending`** > **`ChildSnapshotPending`** > **`Completed`**, **§3-E6**) — `namespacesnapshot_synthetic_tree_test.go` + `namespacesnapshot_synthetic_child_failure_test.go`; **PR5b** — `demovirtualmachinesnapshot_pr5b_test.go` (root **`Completed`** только после готовности child **NSS** по тому же generic пути). Далее PR4 по спеке. **Снятие** synthetic scaffold — сразу после merge-gate (тот же план, §2.4.2).
