...
| `namespacesnapshot_graph_e5_e6_integration_test.go` | **§3-E5 + §3-E6** на **реальном** графе: ручной merge **`childrenSnapshotRefs`** / **`childrenSnapshotContentRefs`** (как domain writer); child берётся как **registered snapshot kind fixture** (NamespaceSnapshot kind, без synthetic-семантики). **E5** — первый root **MCR** не создаётся, пока exclude по subtree нельзя посчитать (child **MCP** / **`manifestCheckpointName`**); root **MCR** не листит **ConfigMap**, уже в child **MCP**; **E6** — **`ChildSnapshotPending`**, приоритет **`SubtreeManifestCapturePending`** vs child pending, **`ChildSnapshotFailed`** при терминальном **`CapturePlanDrift`** на child **MCR**, каскад **`FinalizerParentProtect`** / снятие при удалении parent NSC (см. сценарии в файле) |
...
**N2b:** дерево — **зарегистрированные snapshot-типы** и typed **`NamespaceSnapshot`** как возможный child в refs, **`childrenSnapshotRefs`** / **`childrenSnapshotContentRefs`**, агрегированный **Ready** parent (**§11.1** design; нормативный минимум **§3.8** [`spec/system-spec.md`](../spec/system-spec.md)), **aggregated manifests download** на чтении; **PR4** — нормативный контракт HTTP/ошибок/обхода: [`spec/namespace-snapshot-aggregated-manifests-pr4.md`](../spec/namespace-snapshot-aggregated-manifests-pr4.md); поставка и история PR — [`design/implementation-plan.md`](../design/implementation-plan.md) **§2.4.2**; контракт **E6** и приоритетов reason — интеграция **`namespacesnapshot_graph_e5_e6_integration_test.go`** + unit **`namespace_snapshot_parent_ready_e6_test.go`**; **PR5b** — `demovirtualmachinesnapshot_pr5b_test.go` (root **`Completed`** после готовности дочерних **Demo*Snapshot** по **E6** через registry, **без** искусственного leaf **NamespaceSnapshot** под root). Для demo API parent задаётся через `parentSnapshotRef` (`apiVersion/kind/name`, namespace-local), legacy dual-parent fields удалены. Далее PR4 по спеке.
