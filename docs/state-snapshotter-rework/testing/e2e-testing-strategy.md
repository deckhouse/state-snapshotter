...
| `namespacesnapshot_synthetic_tree_test.go` | **Synthetic child tree** (**временный scaffold в коде, не целевая модель PR5**): child NS; graph refs; parent ждёт child; **`ChildSnapshotPending`** / **`SubtreeManifestCapturePending`** → **`Completed`**; **§3-E5 reconcile-path:** root **MCR** (когда существует) не содержит **ConfigMap**, уже в child **MCP**; без **terminal `CapturePlanDrift`** на пути к parent **Ready**; **контракт каскада:** child NSC имеет `ownerRef`→parent NSC, child с `FinalizerParentProtect`; при delete parent NSC контроллер снимает parent-protect с child (без `Delete` child из этого reconciler) |
