...
| `snapshot_export_test.go` | **Snapshot Export** ([`design/snapshot-export-import.md`](../design/snapshot-export-import.md)): happy path (один data-лист → `VRR` → held PVC под non-controller owner → `DataExport` endpoint в `status.dataSnapshots[].dataURL`, `Ready=Published`) и **fail-closed** при пустом `volumeMode` (`DataReady=False/DataExportFailed`, ни одного `VRR`). Внешние VRR/DataExport симулируются записью `status` (preserve-unknown CRD). |
| `snapshot_static_bind_test.go` | **Static-bind** (`Snapshot.spec.source.snapshotContentName`): bind+mirror Ready на pre-provisioned `SnapshotContent` с обратной ссылкой `spec.snapshotRef`; **misbound** (`SnapshotContentMisbound`, terminal); **missing** content (`SourceContentNotFound`, non-terminal wait). |
| `snapshot_import_test.go` | **Snapshot Import**: pre-seed `index`/per-node `manifests` blob'ов → `DataImport(PVC)` (Ready+UploadFinished) → `VolumeCaptureRequest` (`status.dataRefs[].artifact`=VSC) → pre-provisioned cluster-scoped `SnapshotContent` (`dataRefs[]` + `manifestCheckpointName` + `spec.snapshotRef`) + static-bound root `Snapshot`; **cleanup** populated PVC (DataImport delete → PVC Terminating/NotFound). |
