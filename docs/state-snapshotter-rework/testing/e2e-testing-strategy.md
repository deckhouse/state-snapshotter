...
| `namespacesnapshot_tree_pr2_test.go` | **N2b PR2:** один **synthetic** child (`n2b-pr2-synthetic-tree` на parent, child **`parentName + "-child"`**, label **`n2b-synthetic-child`**); graph refs на root/content; parent **Ready** только после child **Ready**; child без собственных детей |
...
**N2b:** дерево — дочерние NS/NSC, **childrenSnapshotRefs** / **childrenSnapshotContentRefs**, агрегированный **Ready** parent (**§11.1** design), **aggregated manifests download** на чтении (**§8.7**); поставка короткими PR — [`design/implementation-plan.md`](../design/implementation-plan.md) **§2.4.2**; **PR2** — `namespacesnapshot_tree_pr2_test.go`; далее PR3–PR4 по плану.
