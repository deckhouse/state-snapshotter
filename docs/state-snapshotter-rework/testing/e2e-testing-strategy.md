...
| Unit: `namespace_snapshot_parent_ready_e6_test.go`, `child_snapshot_resolve_test.go`; `namespacesnapshot_child_snapshot_watches_test.go` | **[`spec/system-spec.md`](../spec/system-spec.md) §3.2:** дерево run **namespace-local**; **`childrenSnapshotRefs[].namespace`** только **`""`** или совпадение с namespace родителя (**fail-closed** в E6 / **`GetChildSnapshot`**); relay **child→parent** без cluster-wide **`NamespaceSnapshot` list** | N2b E6 / watches | ✅ |
