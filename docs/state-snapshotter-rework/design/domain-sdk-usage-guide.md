...
| `types.go` | `ChildSpec`, `VolumeCaptureSpec`, `ManifestCaptureSpec`, `NotReadyStatus`, `SourceRef`, `DomainCaptureState`, `CoreCaptureState` | DTO, передаёшь в глаголы |
...
- **Suppression по `CoreCaptureState`.** После того как core durably зафиксировал захват
