...
| N1 | **CRD + API:** типы `NamespaceSnapshot`, `NamespaceSnapshotContent`, codegen, OpenAPI; **убрать** использование **generic `SnapshotContent`** как носителя результата для namespace root. | decision + design §14 | ✅ (API + CRD + bootstrap pair; `status.boundSnapshotContentName` — единое поле привязки для snapshot root’ов; N1 reconciler skeleton + integration lifecycle; без ObjectKeeper / полного N2) |
