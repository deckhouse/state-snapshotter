...
|----|--------|----------|-------------------------|-------------------|
| **PR1** | Только **форма графа** в API | JSON: `childrenSnapshotRefs` / `childrenSnapshotContentRefs`; в Go элементы — **`NamespaceSnapshotChildRef`** / **`SnapshotContentChildRef`**. Обновление **design** и при необходимости **spec**; unit / envtest на **сериализацию** | Aggregated download; полный parent content **Ready**; несколько типов детей; domain traversal | «API графа стабилен, дерево в поведении ещё не оживлено» |
