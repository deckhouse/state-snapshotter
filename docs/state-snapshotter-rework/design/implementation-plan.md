...
**Граница §3-E* vs PR5 (demo-domain).** Срезы **§3-E1…E6** ниже — **подготовка generic-инфраструктуры** в модуле (refs, merge, traversal / content refs, dedup, **`Ready`**) поверх нормативного **[`spec/system-spec.md`](../spec/system-spec.md) §3**; они **не** являются этапом «написать демо-доменный контроллер». Первый реальный **domain consumer** контракта — **PR5** ([`demo-domain-dsc/README.md`](demo-domain-dsc/README.md)): DSC, **Demo*Snapshot** / **Content**, запись **`children*Refs`**, прохождение generic **Ready** / dedup / traversal и **снятие** synthetic scaffold по правилу таблицы **§2.4.2**. Минимальный spike одного kind **возможен** уже после **E1/E2**; **практически комфортная** точка входа в PR5 — после **E3/E4**, когда не «плавает» слой traversal / **content refs**. Рекомендуемая нарезка внутри PR5: **PR5a** — один доменный kind (например **DemoVirtualDiskSnapshot**), короткий proof (один DSC, один child path, refs); **PR5b** — при необходимости второй kind и вложенность (например **DemoVirtualMachineSnapshot** → Disk) для проверки промежуточного узла.
...
1. **E3/E4 до рабочего минимума** — зафиксировать поведение вокруг **`childrenSnapshotContentRefs`**; дожать traversal / aggregation настолько, чтобы **domain consumer** не упирался в неясность, **как generic читает граф**; **без** лишнего раздувания матрицы **`Ready`** в этих же PR (широкий **Ready** — **E6** / отдельные изменения).
...
Обход дерева **по refs**; подготовка к aggregated операциям (download / restore по политике N2b); **без** расширения матрицы **Ready** в том же PR (если иначе — разнести). Общий DFS по **`childrenSnapshotContentRefs`** (сортировка детей, циклы) — в коде **`usecase.WalkNamespaceSnapshotContentSubtree`** (агрегатор PR4 и будущий PR5a используют один и тот же ref-only walk).
