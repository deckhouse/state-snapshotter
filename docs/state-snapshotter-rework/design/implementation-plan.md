...
**§3-E5 — dedup / exclude**  
**INV-S0** / **INV-E1**: вычисление только по дереву текущего run; **fail-closed** при неполных данных. **В коде (root capture):** `usecase.BuildRootNamespaceManifestCaptureTargets` + `collectRunSubtreeManifestExcludeKeys` (обход только **`childrenSnapshotContentRefs`** / demo leaves, без list subtree); **`namespacemanifest.FilterManifestTargets`**; wiring в **`namespacesnapshot_capture.go`** + **`ArchiveService`** на **`NamespaceSnapshotReconciler`**. **Ограничения среза:** при пустых **`childrenSnapshotRefs`** exclude не применяется (как раньше — полный namespace list); резолв дочернего snapshot для валидации refs — пока не универсальный GVK, а **`NamespaceSnapshot` / `DemoVirtualDiskSnapshot` / `DemoVirtualMachineSnapshot`** (см. godoc в **`root_capture_run_exclude.go`**). **Follow-up (integration):** end-to-end proof через reconcile (root **`ManifestCaptureRequest.spec.targets`** или архив root **ManifestCheckpoint** после **`Ready`**) ещё не добавлен и вынесен в follow-up PR: при гонке «первый MCR без exclude → затем появляется child MCP → **CapturePlanDrift**» родительский snapshot может не дойти до **`Ready`** без ручного удаления MCR; нужен отдельный PR на устранение гонки вокруг первого MCR перед стабильным integration-тестом.

**Finish line §3-E5 (инженерно честный, рекомендуемый gate перед E6):** (1) **integration proof** — exclude проверяется в реальном reconcile-пути, а не только unit-логикой; (2) **явный выбор и реализация** детерминированного поведения вокруг первого MCR (устранение **CapturePlanDrift** / lifecycle: delayed create MCR, recreate+retry или эквивалент), иначе proof будет флапать или проверять неверное; (3) **универсальный** resolve child snapshot→content — отдельный follow-up, если ближайший объём ограничен тремя типами и это не блокирует (1)–(2). **E6** не ставить следующим обязательным шагом, пока (1) и (2) не закрыты; актуальный статус — [`operations/project-status.md`](../operations/project-status.md).
