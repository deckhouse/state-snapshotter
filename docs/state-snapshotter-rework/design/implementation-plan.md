...
Референс-модель вместо synthetic: под root **`NamespaceSnapshot`** — **heterogeneous** snapshot kinds (demo VM/Disk snapshot + при необходимости CSI **VolumeSnapshot** и `*Content`), **не** цепочка вложенных **`NamespaceSnapshot`**. **Data dedup** (PVC) + **resource dedup** (доменные объекты); **ownerRef** только для иерархии/GC, **coverage** — для dedup. **Этап 1:** документы → ревью → код.
...
- Фиксация перед кодом (этап 2): [`demo-domain-dsc/05-tree-and-graph-invariants.md`](demo-domain-dsc/05-tree-and-graph-invariants.md), [`06-coverage-dedup-keys.md`](demo-domain-dsc/06-coverage-dedup-keys.md), [`07-ready-delete-matrix.md`](demo-domain-dsc/07-ready-delete-matrix.md).
- Расширение **`children*Refs` / PR4 traversal** под не-NS узлы — отдельное решение в spec после апрува дизайна; до апрува пакета **не** считать контракт перенесённым в `spec/system-spec.md`.
