...
**§3-E5 — dedup / exclude**  
**INV-S0** / **INV-E1**: вычисление только по дереву текущего run; **fail-closed** при неполных данных. **В коде (root capture):** `usecase.BuildRootNamespaceManifestCaptureTargets` + `collectRunSubtreeManifestExcludeKeys` (обход **`childrenSnapshotContentRefs`** + dedicated content через registry; **без** list subtree); **`ResolveChildSnapshotToBoundContentNameLive`** / **`ResolveChildSnapshotToBoundContentName`** по **`GVKRegistry`** + опционально один **`TryRefresh`** на операцию при промахе; **`namespacemanifest.FilterManifestTargets`**; wiring в **`namespacesnapshot_capture.go`** + **`ArchiveService`** на **`NamespaceSnapshotReconciler`** (`snapshotgraphregistry.LiveReader`). **Ограничения среза:** при пустых **`childrenSnapshotRefs`** exclude не применяется (как раньше — полный namespace list); при непустых **`childrenSnapshotRefs`** registry **обязателен** (иначе fail-closed). **Живой registry:** `pkg/snapshotgraphregistry` — полная пересборка Current на каждом **`Refresh`** после reconcile DSC **и** не более **одного** **`TryRefresh`** на пользовательскую операцию generic read-path / E5 при пустом registry или **`ErrChildRefNotRegistered`** (без polling). **Finish line (срез):** при непустых **`status.childrenSnapshotRefs`** первый root **MCR** не создаётся, пока exclude по subtree нельзя посчитать полностью (**`ReasonSubtreeManifestCapturePending`**, короткий requeue, зеркалирование на root **NSC**); для **synthetic child tree** **`ensureSyntheticChildSubtreeScaffold`** создаёт child и graph refs **до** первого root MCR, чтобы первый план уже учитывал exclude. **CapturePlanDrift** при subtree-root: удалить **MCR** + requeue (**не** штатный путь к сходимости); для **N2a** без subtree refs — drift по-прежнему **terminal**. Если после exclude не осталось allowlisted целей, root **MCR** с пустыми **`spec.targets`**: **`ManifestCheckpointController`** для **NSS-bound** MCR (аннотация **`bound-namespace-snapshot-content`**) строит пустой checkpoint вместо ошибки «No targets specified». При synthetic child в **терминальном** сбое capture родительский reconcile не перетирает **`ChildSnapshotFailed`** состоянием **`SubtreeManifestCapturePending`**. Integration: **`namespacesnapshot_synthetic_tree_test.go`**, **`namespacesnapshot_e5_subtree_mcr_gate_integration_test.go`**, **`snapshot_graph_registry_dynamic_dsc_test.go`**; unit — **`root_capture_run_exclude_test.go`**, **`root_capture_e5_registry_unit_test.go`**, **`pkg/snapshotgraphregistry/provider_test.go`**.

**§3-E6 — Ready (finish line этого среза)**  
**Единая generic-агрегация** parent **`NamespaceSnapshot`** по **`status.childrenSnapshotRefs`** (только typed child **`NamespaceSnapshot`**, чтение через `client.Reader`, **без** импорта demo API в `internal/usecase`). **Классификация child:** `usecase.ClassifyNamespaceSnapshotChildReady` → completed / pending / failed; терминальные причины **`Ready=False`** child — `usecase.NamespaceSnapshotChildTerminalReadyReasons` (совпадает с whitelist N2a terminal capture). **Отсутствующий** child object (**NotFound**) при уже записанном ref — **pending** (domain может merge ref до создания child NSS). **Приоритет итогового `Ready` reason на parent (строго):** **`ChildSnapshotFailed`** > **`SubtreeManifestCapturePending`** > **`ChildSnapshotPending`** > **`Completed`** — `usecase.PickParentReadyReasonE6`; агрегат по refs — `SummarizeNamespaceSnapshotChildrenRefsWithDefaultNamespace`. **Wiring:** `NamespaceSnapshotReconciler.reconcileChildrenRefsE6ParentReadyOrPatch` (`namespacesnapshot_capture.go`) — после собственного root capture (в т.ч. **`reconcileN2aRootReadyWithoutSynthetic`**), transient subtree/build-root path (**E5**), **`reconcileSyntheticChildTree`**; **`ensureSyntheticChildSubtreeScaffold`** при unbound child — тот же E6-path (**`selfCaptureComplete=false`**). **Synthetic scaffold** в этом срезе **не** удалён. **Тесты:** unit — **`namespace_snapshot_parent_ready_e6_test.go`**; integration — **`namespacesnapshot_synthetic_tree_test.go`** (pending → success), **`namespacesnapshot_synthetic_child_failure_test.go`** (terminal child → parent **`ChildSnapshotFailed`**), **`demovirtualmachinesnapshot_pr5b_test.go`** (root **`Ready=True`/`Completed`** только после готовности child NSS по E6). Мотивы и расширенные таблицы — [`demo-domain-dsc/07-ready-delete-matrix.md`](demo-domain-dsc/07-ready-delete-matrix.md); нормативный минимум — **[`spec/system-spec.md`](../spec/system-spec.md) §3.8**.
