...
| **PR2** | **Один** child в графе **end-to-end** (без доменного writer) | **Было:** временный in-repo scaffold (удалён). **Сейчас:** интеграция **`namespacesnapshot_graph_e5_e6_integration_test.go`** — merge refs, child как **registered snapshot kind fixture** (NamespaceSnapshot kind) + **SnapshotContent/MCP**, parent **Ready** после child; плюс **PR5a** как доменный writer одного kind. | Subtree download как продукт; несколько детей без ручного merge в тесте | «Дерево на **одном** ребёнке работает» |
