...
| **PR2** | **Один** synthetic child **end-to-end** | **Реализация:** opt-in аннотация **`n2b-pr2-synthetic-tree`** на parent; имя child = **`parentName + "-child"`**; label **`n2b-synthetic-child`** (без рекурсии); parent пишет graph refs; parent **Ready** только после child **Ready**; интеграция **`namespacesnapshot_tree_pr2_test.go`**. **Временный scaffold** до **PR5** (не доменный продуктовый flow). | Subtree download; несколько детей; domain wiring | «Дерево на **одном** ребёнке работает» |
| **PR3** | **Политика агрегации Ready** отдельно | **Реализовано:** helper **`evaluateSyntheticRequiredChildStateForPR2`**; parent **`ChildSnapshotPending`** vs **`ChildSnapshotFailed`** vs успех **`Completed`**; allowlist терминальных N2a-причин у child; **§11.1** design (таблица); integration **`namespacesnapshot_tree_pr2_test.go`** (явные reason на pending/success) + **`namespacesnapshot_tree_pr3_test.go`** (child terminal fail → parent **`ChildSnapshotFailed`**). | Aggregated download (PR4); несколько детей; optional/required; domain wiring | «Матрица success / pending / failed зафиксирована тестами» |
...
| **PR5** | Первый **реальный** domain wiring | Подключить одну доменную ветку вместо синтетического child | До PR5 не начинать, пока **PR1–PR4** не зелёные — иначе неясно, баг в графе, агрегации или домене | «Один реальный доменный сценарий на базе стабильного N2b-скелета» |
