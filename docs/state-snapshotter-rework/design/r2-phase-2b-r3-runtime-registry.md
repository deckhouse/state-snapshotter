...
**Целевая формулировка:** CSD изменился → реестр обновлён → **новые** eligible типы начинают watch’иться **без рестарта pod**. Снятие watch и полная симметрия desired/active **не гарантируются**; при необходимости — лог, метрика, рекомендация рестарта.
...
2. ~~**D1–D3**~~ ✅ — [`README.md`](../README.md), [`operations/runbook-degraded-and-unified-runtime.md`](../operations/runbook-degraded-and-unified-runtime.md), [`operations/csd-rbac-and-mcr.md`](../operations/csd-rbac-and-mcr.md). Поддерживать в актуальном виде при изменениях кода.
...
## Модель переходов (на каждый актуальный reconcile CSD / тик реестра)

1. Пересчитать **desired** (bootstrap + актуальный срез CSD).
...
- **Fail-open на уровне процесса, fail-closed на уровне типа:** один битый CSD или один неудачный add не блокирует весь unified-трек; проблемный тип изолируется.

---

## Опасности (не потерять)

- Не блокировать unified-трек из-за одного некорректного CSD.
...
|----------|--------|------------------------|
| CSD стал eligible после перехода **RBACReady=True** → `Sync` → layered state + оба watch зарегистрированы (`active` keys) | ✅ | `test/integration/unified_runtime_hot_add_test.go`, **`Serial`**. Маппинг на **RegistrationTestSnapshot** (изоляция от lifecycle-спек с прямым `Reconcile` на **TestSnapshot**; иначе глобальный watch + ручной Reconcile → 409). Перед тестом удаляются CSD `integration-unified-runtime-hot-add` и `integration-csd-smoke`, чтобы не было **KindConflict** на один snapshot kind. |
...
| Два независимых CSD: один в ошибке, второй корректен | ⬜ | Частично косвенно (KindConflict / InvalidSpec); отдельный интеграционный proof — в бэклоге. |

**Позиционирование proof:** интеграционный hot-add доказывает цепочку **CSD eligible → Sync → обновление registry state (layered + active keys)**, а не полную безопасность «долгоживущей смеси» additive watches в одном envtest manager (watches не снимаются; `active` монотонен).
...
- **`cmd/main.go`:** как раньше: CSD после первичного `SetupWithManager` unified-контроллеров; syncer с bootstrap + `GetAPIReader()` для CSD.
- **`test/integration/setup_test.go`:** для интеграции повторён production-like bootstrap: merge CSD + resolve mapper → `NewGenericSnapshotBinderController` / `NewSnapshotContentController` → `SetupWithManager` → `unifiedruntime.NewSyncer` → `AddCustomSnapshotDefinitionControllerToManager(..., unifiedSyncer.Sync)`. Не раздувать дальше без выноса в хелперы (риск «второй main.go»).
- **`test/integration`:** `csd_api_smoke_test` — маппинг на RegistrationTest*; `controller_registration_test` — без повторного `SetupWithManager` на общем `mgr` (контроллеры уже в BeforeSuite).
