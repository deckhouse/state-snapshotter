...
4. PR5a/PR5b demo-path: child refs пишутся в `children*Refs`; `Ready` root/VM сходится через generic children-readiness aggregation по дочерним snapshot refs. Demo `Ready` для child snapshots пока stub.
