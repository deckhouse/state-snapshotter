...
- Готовность вычисляется **ровно на `SnapshotContent`** (единственный агрегатор, INV-COND2): `SnapshotContent.Ready = ManifestsReady && VolumesReady && ChildContentsReady`.
...
- Failure листа поднимается **только** по ancestor-chain (INV-FAIL1): каждый предок получает `ChildContentsReady=False` / `Ready=False` / reason `ChildrenFailed`, а `message` сохраняет имя failed-потомка и исходный reason/message (кумулятивно по глубине); sibling-ветки не затрагиваются.
...
2. Каждый предок-`SnapshotContent` при агрегации видит ребёнка `Ready=False` → выставляет `ChildContentsReady=False` / `Ready=False` (reason `ChildrenFailed`) и сохраняет имя/первопричину в `message`.
