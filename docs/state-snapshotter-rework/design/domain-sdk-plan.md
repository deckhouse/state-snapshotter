...
pkg/snapshotsdk/                   # КАНОНИЧЕСКИЙ root-модуль (Р2): весь type+protocol API виден здесь (litmus, §1)
  ...
  *.go                             #   CaptureSDK (+ роли ReadinessFault/Planning/PlanningBarrier), SnapshotAdapter,
                                   ...
                                   #   Reason, sentinel-ошибки. Весь capture-lifecycle (TOCTOU live-refresh,
                                   #   marker-suppression, MCR/VCR orchestration) — здесь. SourceValidator — доменный (Р22/Р28), не тут.
...
`MarkPlanningFailed(ReasonManifestDrift)`. Technical owned-PVC target из захвата данных добавляется в desired
**до** сравнения (иначе false-positive). Тесты (`pkg/snapshotsdk` + domain-controller): create publishes
targets; restart с тем же set (другой порядок) → no-op; `[A,B] → [A]` и `[A,B] → [A,C]` → `ErrManifestDrift`
(existing не изменён); owned-PVC augmentation не даёт false drift; `ErrManifestDrift` маппится в
`MarkPlanningFailed(ReasonManifestDrift)`. Итог: все три части планирования — **fail-closed**, SDK не
self-heal-ит, но **репортинг ошибки различается по типу захвата**: дочерние снапшоты и захват манифестов
дают sentinel + терминальный `MarkPlanningFailed(reason)` (`ReasonTopologyDrift`/`ReasonManifestDrift`),
а захват данных (VCR-drift) — обычную ошибку на retry-пути (sentinel/reason нет).
...
**Исключение — топология детей фиксируется на коммите барьера (Р23):** до `MarkPlanningReady` child-set
подчиняется «converge-to-current-desired» как и data/manifest. Но как только барьер закоммичен
(`ChildrenSnapshotReady=True`), набор детей иммутабелен: recompute после рестарта, давший другой child-set
(включая отрастание ребёнка у закоммиченного листа `[] → [A]`), — это `ErrTopologyDrift` (fail-closed), а не
повод переписать топологию. Конвергенция к новому desired остаётся допустимой для data/manifest-плеч и
после коммита (их иммутабельность держится на immutable `spec.sourceRef`, а не на барьере).
...
- статус-поля `api/demo/v1alpha1`: `DomainCaptureState{ManifestCaptureRequestName, VolumeCaptureRequestName,
  ChildrenSnapshotRefs}` и `CoreCaptureState{ManifestCaptured, DataCaptured}` (core-handoff, который SDK
  читает для suppression) — соответствуют реальному `DemoVirtualDiskSnapshotStatus`/`...MachineSnapshotStatus`
  (Р21 split корректен);
...
// Домен реализует его один раз и передаёт во ВСЕ методы facade. Ownership разведён по сигнатурам (Р21):
// domain-state read+write, core-state только getter. (Р15.5: SDK-owned Condition-обёртка рассматривалась;
// v1 отдаёт metav1.Condition напрямую.)
type SnapshotAdapter interface {
    Object() client.Object                       // мост к controller-runtime
    SourceRef() SourceRef                         // spec.sourceRef (identity — домен)
    GetConditions() []metav1.Condition            // SDK мерджит по одному условию, штампует observedGeneration
    SetConditions([]metav1.Condition)
    GetDomainCaptureState() DomainCaptureState    // domain-owned: read+write
    SetDomainCaptureState(DomainCaptureState)
    CoreCaptureState() CoreCaptureState           // core-owned: ТОЛЬКО чтение (сеттера нет — misuse невозможен по типам)
}
...
type Planning interface {
    ...
    // create-or-adopts) and derives the published SnapshotChildRef. Topology is immutable after the barrier
    // (MarkPlanningReady → ChildrenSnapshotReady=True): a desired set diverging from the published one (set
    // equality on (apiVersion,kind,name), not count) is terminal ErrTopologyDrift — fail-closed, creates/
    // deletes nothing. A duplicate desired child is a non-drift error. nil/empty = leaf ("no children"),
    // NOT "discovery failed"; on discovery failure call MarkPlanningFailed instead.
    ...
    // (delete-free: never clears a published name). Suppressed once core stamped DataCaptured. An existing
    // VCR whose target differs fails closed (generic error, retry path) before adoption.
    ...
    // The FINAL set must be non-empty, else ErrEmptyManifest before any cluster mutation. An existing MCR
    // whose targets differ (set equality) is terminal ErrManifestDrift — fail-closed, no update/patch/delete.
    // Suppressed once core stamped ManifestCaptured. Does not mark planning ready.
...
// Sentinel-ошибки fail-closed путей (домен маппит в MarkPlanningFailed(reason, …)):
//   ErrTopologyDrift — EnsureChildren: desired child set != published после барьера → ReasonTopologyDrift.
...
// New строит CaptureSDK из client (записи/кэш-чтения) + APIReader (live-refresh маркеров) + VolumeCaptureProvider.
