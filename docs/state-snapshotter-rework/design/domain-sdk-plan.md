...
Три ноги — это три ортогональные части узла. Таблица в §0.3 — не «8 случайных шагов», а **реализация
этих трёх ног** + вход/барьер.
...
| 4 | узел имеет детей (parent, напр. VM) | отдаёт SDK **полный desired-set детей** одним вызовом (как собрал — приватно, Р17) | материализовать/синхронизировать дочерние snapshot-CR + `childrenSnapshotRefs` (replace + orphan-GC, Р23) | `EnsureChildren(ctx, adapter, desired)` (leaf: nil/empty **only after successful discovery**, Р23) |
...
| 6 | всегда (у любого узла есть manifest) | просит обеспечить **MCR** | захватить манифест узла; targets выводятся из VCR при наличии data-leg | `sdk.EnsureManifestCapture(...)` |
...
> Несколько независимых data-ног на одном узле в SDK v1 **не поддерживаются** — такой случай
...
pkg/snapshotsdk/                   # КАНОНИЧЕСКИЙ root-модуль (Р2): весь type+protocol API виден здесь (litmus, §1)
  ...
  *.go                             #   CaptureSDK (+ узкие роли/targets), SnapshotAdapter, SourceValidator,
  ...
  internal/                        # low-level mechanics (Р15.6), НЕ публичный SDK surface
    children/        # ensure + build/sort/dedup/compare childrenSnapshotRefs + orphan-GC (transport mechanics)
...
- **`children/` → `internal/`** — sort/dedup/compare/orphan-GC childrenSnapshotRefs = transport mechanics;
...
7. orphan-GC (Р23): `EnsureChildren` с уменьшенным desired-set удаляет свои orphan child-CR; пустой set
   удаляет все прежние SDK-owned children; повтор — no-op.
...
определяет только, **как дети публикуются в snapshot-протокол** (create/adopt + ownerRef + refs + orphan-GC),
и публикация — **desired-set based и атомарна** с точки зрения протокола.
...
sdk.EnsureChildren(ctx, adapter, children)       // SDK: create/adopt + ownerRef + sort/dedup + write childrenSnapshotRefs + orphan-GC
```
...
- **replace semantics** — status родителя отражает актуальный desired-set, а не историю добавлений
  (исчез ребёнок из доменной модели → SDK видит, что его больше нет, и удаляет orphan child-CR, Р23);
...
- `EnsureChildren(ctx, t, desired)` — full reconciliation desired-set: ensure дочерних snapshot-CR,
  **удаление orphan'ов среди своих child-CR** (Р23), sort/dedup, запись `childrenSnapshotRefs`. Для leaf
  без детей — `EnsureChildren(ctx, t, nil)` (пустой set = позитивное «детей нет» → удалит все прежние
  SDK-owned children). **MUST (Р23):** звать только после успешного discovery; при сбое discovery —
...
- **SDK владеет:** create/adopt, ownerRef, деривация `SnapshotChildRef` из объекта (single source of truth →
  без drift «Ref сказал X, Object назван Y»), sort/dedup/публикация `childrenSnapshotRefs`, orphan-GC (Р23).
...
### Р23. `EnsureChildren` — full reconciliation **включая orphan-deletion** (РЕШЕНО)
Раньше судьба «исчезнувшего» ребёнка (reconcile: `A B C` → `A B`, `C` пропал) была неоднозначна, а без
удаления **replace-семантика desired-set неполна**: status отражает desired-set, а K8s-объекты — нет.
**Решение:** `EnsureChildren` выполняет **полную реконсиляцию desired-set, включая удаление orphan'ов** —
дочерних snapshot-CR, **которые SDK сам материализовал/владеет** (по ownerRef), и которых больше нет в
desired-set. Границы: SDK удаляет только **свои** child snapshot-CR (не произвольные объекты), уважает
finalizers, операция идемпотентна и crash-safe (Р19: после рестарта пересчёт desired-set сходится к
тому же множеству, повторное удаление — no-op). Domain owns *какие* дети; SDK owns *материализацию +
publication + GC orphan'ов* среди своих child-CR.

**Опасность `nil`/empty — обязательный инвариант (MUST).** `EnsureChildren(nil)` означает не «discovery
failed», а **«leaf/детей нет» → «удали всех прежних SDK-owned children»**, поэтому `nil`/empty НЕ должен
использоваться как «я не смог собрать детей». Иначе сбой discovery молча снесёт уже существующих детей. Контракт:
- Domain MUST call `EnsureChildren` **only after successful child discovery**.
- If child discovery fails, domain MUST call `MarkPlanningFailed(cause)` and **MUST NOT** call
  `EnsureChildren` с `nil`/empty desired-set.
- empty desired-set — это **позитивное утверждение** «у узла нет детей» (leaf), а не «discovery не дал
  результата». Эти два состояния домен обязан различать **до** вызова SDK.
Это **главный риск** desired-set + orphan-GC модели и **MUST** быть явно отражён в godoc `EnsureChildren`
(Р18): что делает с orphan'ами и что `nil` = «детей нет», а не «ошибка discovery».
...
дообеспечивает недостающее (children → orphan-GC, Р23; ровно один VCR; MCR) и **безопасен после рестарта
...
T8  EnsureChildren([A,C]) MUST:  keep/reuse A; create/ensure C;
                                 delete SDK-owned orphan B; refs → [A,C]
...
### Р29. Orphan-GC без desired GVK: diff против durable `childrenSnapshotRefs` (РЕШЕНО)
...
| `vm_controller.go`: паттерн `ensure дочернего снапшота` + публикация refs (логика «какие дети» остаётся в демо) | generic ensure-child + build refs + orphan-GC (Р23) | `internal/children` |
...
2. **Child-материализация домен-типизирована — РЕШЕНО (Р17.5): child-builder seam.**
   ...
   решение), SDK владеет create/adopt + ownerRef + дериваця `SnapshotChildRef` + sort/dedup + orphan-GC.
   Opaque spec (`map[string]any`) отвергнут (architectural leak; теряет типобезопасность; slippery slope) —
   обоснование в Р17.5.
3. **Orphan-GC (Р23) — аддитивен, для demo это no-op; механика — diff vs durable status (Р29).** Текущее
   демо НЕ удаляет orphan child-CR, а VM ссылается на один диск (`vm.Spec.VirtualDiskName`), поэтому orphan
   не возникает. На `EnsureChildren(nil)` desired GVK взять неоткуда → orphan-GC диффает desired против
   durable `status.childrenSnapshotRefs` (`orphans = previous − desired`, адресная delete по
   apiVersion/kind/name), **РЕШЕНО Р29**. Чтобы S1 остался **no-op refactor**: orphan-GC — новая SDK-гарантия,
   проверяется **новым** conformance-тестом, поведение demo не меняется.
4. **`VolumeCaptureSpec` несёт targets, посчитанные доменом; missing-PVC — доменный.** `EnsureVolumeCapture`
   = чистый ensure VCR. Резолв PVC (`APIReader`), сборка `vcpkg.Target`, терминальный `ReasonArtifactMissing`
   + requeue — остаются в домене (это «что захватывать»).
5. **`ManifestCaptureSpec` несёт target identity (apiVersion/kind/name) + опц. `vcrName`.** MCR-targets
   деривируются из VCR (`demoManifestCaptureTargetsFromVCR`/`manifestTargetsFromVolumeTargets`) — это часть
   ensure-manifest в SDK, но identity-вход даёт домен.
...
// children + orphan-GC (Р17/Р23) / ≤1 data slot / content-free / MarkPlanningReady = barrier (не
...
type Planning interface {
    ...
    // SnapshotChildRef from it. It uses replace semantics: it ensures every desired child and
    // deletes orphaned SDK-owned child snapshots no longer in the set. It does not mark planning ready.
    //
    // WARNING: a nil/empty desired set means "this node has no children" (leaf) and will
    // DELETE all previously published SDK-owned children. It does NOT mean "discovery failed".
    // Callers MUST call EnsureChildren only after successful child discovery; on discovery
    // failure call MarkPlanningFailed instead and never pass nil/empty as a failure signal.
...
// internal/children:  ensure/sort/dedup/compare childrenSnapshotRefs + orphan-GC (Р23).
...
- **Нет** поддержки нескольких независимых data-ног на одном узле — *multiple independent data
...
- Consistency-аудит (ревью 4b, устранение протечек facade):
  ...
  - Р23 (**РЕШЕНО**): `EnsureChildren` — full reconciliation desired-set **включая orphan-deletion**
    своих SDK-owned child-CR (по ownerRef, уважая finalizers); иначе replace-семантика неполна.
    **MUST-инвариант:** `nil`/empty = «детей нет» (удалит всех прежних children), НЕ «discovery failed»;
    звать только после успешного discovery, при сбое — `MarkPlanningFailed`. Отражено в godoc (Р18).
...
- Consistency-аудит (ревью 5, design vs фактический код — §5.1a):
  - Р29 (**РЕШЕНО**): orphan-GC без desired GVK — diff против durable `status.childrenSnapshotRefs`
    (`orphans = previous − desired`, адресная delete по apiVersion/kind/name). Crash-safe, работает для
    empty desired-set, не требует enumeration kind'ов; list-by-ownerRef — только optional hardening.
...
- `pkg/snapshotsdk/internal/children` — ensure child CRs, sort/dedup refs, orphan-GC;
...
- `EnsureChildren`: принимает полный desired-set `[]ChildSpec` (Р17.5: `ChildSpec.Object` — домен-built);
  ...
  sort/dedup refs; orphan-GC = diff против durable `status.childrenSnapshotRefs` (`previous − desired`, Р29);
...
- сохранить доменную логику: выбор детей + **построение child-объектов** (`DemoVirtualDiskSnapshot`) для
  `ChildSpec.Object` (Р17.5), и PVC targets для `VolumeCaptureSpec`.
...
4. `internal/children`: `EnsureChildren([A,C])` после `[A,B]` удаляет SDK-owned orphan `B`.
...
5. `internal/children`: `EnsureChildren(nil)` удаляет все прежние SDK-owned children.
5a. `internal/children`: orphan-GC через durable refs (Р29) — `previous=status.childrenSnapshotRefs`,
    desired без объекта удалённого ребёнка → delete по apiVersion/kind/name; на empty desired GVK не нужен.
...
- добавлены conformance-тесты: D4a ownership, content-free, deterministic refs, TOCTOU/restart, orphan-GC.
