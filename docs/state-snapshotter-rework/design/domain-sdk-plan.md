...
`MarkPlanningFailed(ReasonManifestDrift)`. Итог: захват манифестов, захват данных и дочерние снапшоты —
все **fail-closed**, SDK не self-heal-ит.
...
> **Статус реализации v1 (СИНХРОНИЗАЦИЯ с кодом):** в v1 методы facade (`Ensure*`/`Mark*`) принимают
> **полный `SnapshotAdapter`**, а не узкие target-интерфейсы. Узкие targets (`ManifestCaptureTarget`/
> `VolumeCaptureTarget`/`ChildrenTarget`/`BarrierTarget`/`ReadinessFaultTarget`) — это **отложенное**
> nice-to-have для тестируемости (Р24/Р27), не реализованное в v1. Причина: адаптер строится один раз и
> является единственным domain↔SDK seam; раздача узких ролей по сигнатурам не добавляет безопасности (тот
> же объект удовлетворяет всё), а только умножает типы. `Planning`/`PlanningBarrier`/`ReadinessFault` как
> композируемые **роли facade** (Р24) реализованы и остаются точкой для focused-моков. Если в будущем
> понадобится строгая сегрегация по входам — вводить узкие targets отдельным изменением, синхронно с этим
> разделом.
...
// инварианты: 3 части планирования / crash-restart safety + converge-to-current-desired (Р19/Р25) / desired-set
...
type Planning interface {
    ...
    // EnsureVolumeCapture ensures the optional data capture for the snapshot node.
    ...
    // EnsureManifestCapture ensures the manifest capture for the snapshot node.
...
type PlanningBarrier interface {
    ...
    // It does not roll back already published planning.
