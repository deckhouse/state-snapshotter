...
## 0. Что доменный контроллер делает: три части планирования + reconcile flow
...
### 0.1 Ментальная модель: capture узла = три независимые части планирования

> **From the domain controller point of view, capture of one snapshot node has three independent
> parts:**
> 1. **Manifest capture — always present.** Domain asks SDK to ensure **MCR** for this node.
> 2. **Data capture — optional, occupancy 0..1.** If the node has data, domain asks SDK to ensure
>    **exactly one VCR**. If the node is manifest-only, there is no data capture.
> 3. **Child snapshot planning — optional, 0..N.** Domain discovers children **privately** and
>    publishes the **complete desired child set** through SDK.
>
> Planning is complete only after manifest capture, data capture (when applicable), and child snapshot
> planning are all published. Then domain marks the node as **planning-ready**.

> **NB про момент «freeze» (Р23/Р25):** «published» в этом нарративе **≠ immutable**. Опубликованный
> child-set становится иммутабельным **не** в момент `EnsureChildren`, а на **коммите барьера**
> (`MarkPlanningReady` → `ChildrenSnapshotReady=True`). До коммита set MAY сходиться к новому desired; после
> коммита расхождение — `ErrTopologyDrift`. Симметрично: захват данных иммутабелен по immutable
> `spec.sourceRef`, захват манифестов — fail-closed на `ErrManifestDrift`. То есть «planning complete after
> everything is published, then mark ready» — это про **порядок**, а freeze-point у всех трёх частей — это
> **барьер**.

Это ровно **canonical graph model** (§1, §6): `node = manifest + one logical data slot + children[]`.
Эти три части планирования — три ортогональные части узла. Таблица в §0.3 — не «8 случайных шагов», а
**реализация этих трёх частей** + вход/барьер.

> **Ключевой инвариант ответственности:** `SDK does not own domain decisions. SDK owns publication of
> three protocol parts: manifest, data slot, children.`
> - `MCR` — это **захват манифестов**; `VCR` — это **захват данных**; `EnsureChildren` — это **дочерние
>   снапшоты**;
> - `MarkPlanningReady` — **барьер после публикации всех применимых частей**.

### 0.2 Lifecycle узла: Ensure* (do planning) → Mark* (commit outcome)

Доменный контроллер планирует эти три части, затем **обязан** завершить planning phase одним из **двух
planning outcomes** (исход именно planning-фазы, а не lifecycle объекта — поэтому «planning outcome», не
«terminal»):

```text
// sdk — инстанс snapshotsdk.CaptureSDK, инъектированный в reconciler (true facade, Р26)
1. Manifest capture → sdk.EnsureManifestCapture(...)   // ensure MCR
2. Data capture(0..1) → sdk.EnsureVolumeCapture(...)   // ensure exactly one VCR (если есть данные)
3. Child snapshots(0..N) → sdk.EnsureChildren(...)     // publish complete desired child set
                         ↓
   outcome: sdk.MarkPlanningReady(...)  ИЛИ  sdk.MarkPlanningFailed(...)
```
...
прогресса); до `MarkPlanningReady` опубликованные части реконсилируются к **новому** observed desired (Р25).
...
- **домен решает:** что является source'ом снапшота; какие у узла дети; какие PVC/тома попадают в
  захват данных; имена/kind'ы домена; когда узел вообще нужно снапшотить.
...
Таблица ниже — реализация трёх частей (§0.1): шаги 4/5/6 = дочерние снапшоты / захват данных / захват
манифестов, шаг 7 = барьер.
...
| 3 | — (нет публичного шага) | ничего; **просто вызывает `Ensure*`** | suppression (не пересоздавать `MCR`/`VCR`) — **internal** механика SDK по барьеру (Р31), не public API (Р20) | _(internal; `AlreadyCaptured` убран из facade)_ |
...
| 5 | узел снимает данные (leaf, напр. Disk) | просит обеспечить **VCR** для своих томов | запустить захват данных тома (через backend-порт), occupancy слота = 1 | `sdk.EnsureVolumeCapture(...)` |
| 6 | всегда (у любого узла есть manifest) | отдаёт SDK **полный desired-set manifest targets** | создать один MCR для manifest targets узла; technical owned-PVC target добавляется из VCR при наличии захвата данных | `sdk.EnsureManifestCapture(...)` |
...
| 8 | барьер закоммичен (`ChildrenSnapshotReady=True`) | ничего не пересоздаёт; reconcile идемпотентен | завершить planning без дублей; узел в steady state | (шаги 4–6 **inert**: `Ensure*` → `nil`, Р31) |
...
- `boundSnapshotContentName` пишет **core**, домен его только читает; suppression пересоздания делает SDK по
  барьеру (`ChildrenSnapshotReady=True`), а не по core-маркерам (Р31: `manifestCaptured`/`dataCaptured` удалены);
...
- подавление пересоздания запросов по барьеру планирования (`ChildrenSnapshotReady=True`, Р31);
- детерминированные/отсортированные/дедуплицированные `childrenSnapshotRefs`;
- publish-gated иммутабельность: опубликованный артефакт фиксируется (drift → fail-closed), Р31;
...
> Слот может быть:
> - **empty** — узел без данных, manifest-only (например VM): occupancy = 0;
...
> Несколько независимых захватов данных на одном узле в SDK v1 **не поддерживаются** — такой случай
> (гипотетический multi-disk VM) декомпозируется в дочерние узлы (children), а не в один узел.
>
> Заметка по терминологии: канон — абстракция **logical data slot**, а НЕ имя поля. Точное имя поля
...
- **Создание data-capture VCR** на PVC через unstructured+GVK — `disk_volume_capture.go`,
...
- **Подавление пересоздания** по барьеру `ChildrenSnapshotReady=True` (Р31; прежние core-маркеры
  `status.manifestCaptured`/`status.dataCaptured` и live-refresh через `APIReader` удалены).
...
- Поля core (домен только читает): `boundSnapshotContentName`, `Ready` (Р31: `manifestCaptured`/`dataCaptured` удалены).
...
pkg/snapshotsdk/                   # КАНОНИЧЕСКИЙ root-модуль (Р2): весь type+protocol API виден здесь (litmus, §1)
  ...
  *.go                             #   CaptureSDK (+ роли ReadinessFault/Planning/PlanningBarrier), SnapshotAdapter,
                                   #   ChildSpec, ManifestCaptureSpec, VolumeCaptureSpec, NotReadyStatus, VolumeCaptureProvider,
                                   #   Reason, sentinel-ошибки. Весь capture-lifecycle (barrier-suppression Р31,
                                   #   publish-gated drift, MCR/VCR orchestration) — здесь. SourceValidator — доменный (Р22/Р28), не тут.
...
5. barrier-suppression (Р31) предотвращает дублирующий MCR/VCR после коммита; publish-gated drift — до;
...
type SnapshotAdapter interface {
    ...
    GetConditions() []Condition; SetConditions([]Condition)             // SDK-owned Condition, НЕ metav1 (Р15.5); барьер
}
// Р31: прежний core-owned getter `CoreCaptureState()` удалён — suppression по барьеру (см. Р31).
```
...
(barrier-suppression Р31, publish-gated drift, MCR/VCR orchestration), живёт в **root** `pkg/snapshotsdk`
...
4. **Value objects / iota-enums вместо «голого» `bool`/`string`**, когда у значения есть фиксированный
   ...
   вводим enum-состояние — planning outcome выражается **глаголами** `MarkPlanningReady`/
...
**lifecycle capture-планирования** (захват манифестов → optional захват данных → optional дочерние снапшоты
→ итоговый planning outcome, §0), поэтому публичная поверхность SDK должна **отражать именно этот протокол**.
...
- какие операции — **planning work** (`Ensure*`), а какие — **planning outcome** (`Mark*`).
...
planning-completion transitions (planning ready / planning failed); (г) guarantees и ограничения реализации.
...
**одна точка входа**, где виден весь lifecycle capture-планирования: захват манифестов → optional захват
данных → дочерние снапшоты → planning outcome `MarkPlanningReady`/`MarkPlanningFailed` (§0).
...
**Форма** (composition of facade roles `ReadinessFault`/`Planning`/`PlanningBarrier`, Р24; `AlreadyCaptured`
убран в internal, Р20; `ValidateSource` убран из facade, Р28; точные сигнатуры — §5.3):
```go
...
// все методы принимают единый SnapshotAdapter (Р27-вердикт); данные операции — через DTO
```
...
**Godoc на `CaptureSDK` MUST** (Р18) описывать весь lifecycle и инварианты: захват манифестов, захват данных
и дочерние снапшоты; crash/restart safety (Р19); desired-set semantics для children (Р17); ≤1 logical data
slot (occupancy
...
if err := validator.Validate(adapter.SourceRef()); err != nil {  // доменный SourceValidator (Р22)
    _ = sdk.MarkNotReady(ctx, adapter, NotReadyStatus{Reason: "InvalidSourceRef", Cause: err})  // SDK: publish исход (Р30)
}
```
То есть identity-проверка — чисто domain concern, `MarkNotReady` — transport/publication SDK (Р30).

### Р17. Capture = три части планирования; children desired-set; publish ≠ barrier (РЕШЕНО)
**Ментальная модель (см. §0.1):** capture одного узла = **три независимые части планирования** —
...
`SDK does not own domain decisions. SDK owns publication of three protocol parts.`

**Граница ответственности (дочерние снапшоты):** `Domain owns child planning (discovery + building child
...
`Mark* = commit planning outcome` (только фиксирует исход). После публикации всех применимых частей домен
обязан завершить planning phase **бинарным** planning outcome: `MarkPlanningReady` (completed) или
...
if err := validator.Validate(adapter.SourceRef()); err != nil {      // source identity — domain (Р28)
    return sdk.MarkNotReady(ctx, adapter, NotReadyStatus{Reason: "InvalidSourceRef", Cause: err}) // Ready=False, stop (Р30)
}
...
if err := sdk.EnsureVolumeCapture(ctx, adapter, volumeSpec); err != nil { // только если узел снимает данные
    return sdk.MarkPlanningFailed(ctx, adapter, err)
}
...
**Top-level doc capture-пакета MUST** описывать **полный planning lifecycle** (§0): захват манифестов →
optional захват данных → дочерние снапшоты → итоговый planning outcome — как **единую точку** canonical
...
// It assumes all applicable planning is already published:
...
- `MarkPlanningReady` допустим **только** после того, как всё применимое планирование **durably published**.
...
закоммиченный барьер и все `Ensure*` станут inert (Р31), дублей не создаст.
...
facade потёк бы. **Решение:** `AlreadyCaptured` **НЕ** входит в public `CaptureSDK`. Домен его не зовёт:
`Ensure*` и так идемпотентны (Р19). Caller просто вызывает `Ensure*` — «спрашивать already captured?» не нужно.
(Если когда-то понадобится публичный прогресс — отдать **semantic object** `CaptureProgress`, НЕ два `bool`;
но в v1 не вводим.) **Уточнение Р31:** suppression теперь по барьеру (`ChildrenSnapshotReady=True`), а не по
core-маркерам, и live-refresh для маркеров больше не нужен (артефакты-планирования publish-gated).

### Р21. Split `CaptureStatus` по ownership (ИСТОРИЯ; core-half отменён в Р31)
Один DTO на двух writers размывает ownership (домен теоретически мог бы выставить core-поле). Разводили
**типами**, чтобы ownership был enforceable:
```go
type DomainCaptureState struct {            // domain read+write (ДЕЙСТВУЮЩИЙ)
    ManifestCaptureRequestName string
    VolumeCaptureRequestName   string
    ChildrenSnapshotRefs       []SnapshotChildRef
}
type CoreCaptureState struct {              // core-owned — ОТМЕНЁН (Р31)
    ManifestCaptured         bool
    DataCaptured             bool
    BoundSnapshotContentName string
}
```
**Действующее (Р31):** `DomainCaptureState` остаётся; `CoreCaptureState` и `CoreCaptureState()`-getter
**удалены** — suppression теперь по барьеру, а не по core-маркерам. Исходно адаптер имел `CoreCaptureState()`
только-getter; в v1 этого метода нет.
...
**Дополнение (publish-gated иммутабельность + fail-closed на drift — действующее решение, см. Р31).** Раз
orphan-deletion снят, у `EnsureChildren` две обязанности: (1) create/adopt desired и (2) проверка инварианта
«опубликованный артефакт иммутабелен». Барьер (`ChildrenSnapshotReady=True`, пишет `MarkPlanningReady`) —
**финальный commit point** фазы планирования, но отдельный артефакт иммутабелен **с момента публикации**,
ещё до барьера. Три состояния:

- **State 1 — ничего не опубликовано, барьер не закоммичен:** desired-set свободно сходится (Р25), drift не
  диагностируется. Пустой опубликованный набор детей — тоже State 1 (ещё может сойтись).
- **State 2 — опубликовано, барьер не закоммичен:** опубликованный НЕпустой child-set обязан совпасть с desired
  по **множественному равенству `(apiVersion, kind, name)`, НЕ по count** (равный count с другим членом — drift).
  Расхождение — **терминальный** `ErrTopologyDrift`: ничего не создаёт/не удаляет, refs не трогает; домен
  публикует `MarkPlanningFailed(ReasonTopologyDrift)`. Это закрывает дыру «молчаливой перезаписи intent» после
  рестарта с недетерминированным discovery.
- **State 3 — барьер закоммичен:** SDK **inert** — `EnsureChildren` сразу возвращает `nil`, ничего не создаёт,
  не переиспользует и не валидирует; расхождение после коммита — невалидное состояние, которое SDK не чинит и
  не репортит (владение перешло core-контроллеру).
...
только проверяет инвариант и **fail-closed**. Тот же принцип — на data-слоте: VCR фиксируется на первом
planning из иммутабельного `spec.sourceRef`, существующий VCR с другим target до барьера fail-closed,
`EnsureVolumeCapture(nil)` ничего не чистит. Полный набор drift-сценариев (set-equality, State 2/3, дубликаты)
— в `domain-sdk-test-plan.md` и тестах `pkg/snapshotsdk`.

**Захват манифестов — симметрично fail-closed (тот же State 1/2/3).** До барьера, после первой публикации,
`EnsureManifestCapture` сравнивает desired target-set с уже опубликованным MCR по множеству
`(apiVersion, kind, name)` (порядок/дубликаты не важны) и при расхождении возвращает `ErrManifestDrift`,
**не** патча/не пересоздавая/не удаляя запрос и не трогая status; домен публикует исход через
`MarkPlanningFailed(ReasonManifestDrift)`. После коммита барьера метод **inert** (как и остальные). Technical
owned-PVC target из захвата данных добавляется в desired **до** сравнения (иначе false-positive). Тесты
(`pkg/snapshotsdk` + domain-controller): create publishes targets; restart с тем же set (другой порядок) →
no-op; `[A,B] → [A]` и `[A,B] → [A,C]` → `ErrManifestDrift` (existing не изменён); owned-PVC augmentation не
даёт false drift; `ErrManifestDrift` маппится в `MarkPlanningFailed(ReasonManifestDrift)`. Итог: все три части
планирования — **fail-closed pre-commit, inert post-commit**, SDK не self-heal-ит, но **репортинг ошибки
различается по типу захвата**: дочерние снапшоты и захват манифестов дают sentinel + терминальный
`MarkPlanningFailed(reason)` (`ReasonTopologyDrift`/`ReasonManifestDrift`), а захват данных (VCR-drift) —
обычную ошибку на retry-пути (sentinel/reason нет).

**Manifest cardinality (инвариант).** После SDK-augmentation финальный manifest target-set обязан содержать
**минимум один** target. Пустой захват манифестов — невалиден и приводит к ошибке SDK (`ErrEmptyManifest`),
возвращаемой **до** любых cluster-мутаций (нет MCR Get/Create, нет status patch). Важно: **SDK не inject-ит
source object** — обязанность передать хотя бы один manifest target (как минимум сам снапшотируемый ресурс)
остаётся на доменном контроллере; owned-PVC из захвата данных может «спасти» пустой вход, но это не замена
...
type CaptureSDK interface { ReadinessFault; Planning; PlanningBarrier }
```
Доменный тест мокает только нужную роль (напр. `Planning`), а не весь facade. Эти роли реализованы в v1.
(Более мелкие per-method target-интерфейсы — отдельный вопрос Р27, отложенный в v1.)
...
«с памяти»). Until `MarkPlanningReady` is durably published, desired data/manifest **MAY** be reconciled to
the newly observed domain state.

**Исключение — publish-gated иммутабельность (Р23/Р31):** «converge-to-current-desired» действует только пока
артефакт **не опубликован** (State 1). Как только child-set/MCR/VCR опубликованы, но барьер ещё не закоммичен
(State 2), recompute после рестарта, давший другой набор, — это drift (fail-closed: `ErrTopologyDrift`/
`ErrManifestDrift`/VCR-error), а не повод переписать intent. После коммита барьера (State 3) все `Ensure*`
inert и drift не диагностируют. Отрастание ребёнка у закоммиченного листа `[] → [A]` после барьера —
проигнорировано (inert), а не drift.
...
children, err := planChildren(ctx, source)                           // domain planning: строит []ChildSpec (Р17.5)
if err != nil {                                                      // planning упал → НЕ EnsureChildren(nil)
    return sdk.MarkPlanningFailed(ctx, adapter, ReasonGraphPlanningFailed, err)
}
if err := sdk.EnsureChildren(ctx, adapter, children); err != nil {
    reason := ReasonCreateChildFailed
    if errors.Is(err, ErrTopologyDrift) { reason = ReasonTopologyDrift }
    return sdk.MarkPlanningFailed(ctx, adapter, reason, err)
}
if err := sdk.EnsureVolumeCapture(ctx, adapter, volumeSpec); err != nil {
    return err                                                       // generic error → retry (data-capture drift fail-closed)
}
if err := sdk.EnsureManifestCapture(ctx, adapter, manifestSpec); err != nil {
    if errors.Is(err, ErrManifestDrift) {
        return sdk.MarkPlanningFailed(ctx, adapter, ReasonManifestDrift, err)
    }
    return err
}
return sdk.MarkPlanningReady(ctx, adapter, "planning complete")
```
...
API (`CaptureSDK` + роли `ReadinessFault`/`Planning`/`PlanningBarrier`, `SnapshotAdapter`, `ChildSpec`,
`ManifestCaptureSpec`, `VolumeCaptureSpec`, `NotReadyStatus`, `VolumeCaptureProvider`, `Reason`, sentinel-ошибки).
...
### Р27. Узкие target-интерфейсы в сигнатурах методов (РАССМОТРЕНО → ОТЛОЖЕНО в v1)
Идея Р24 в пределе: каждый метод берёт не весь `SnapshotAdapter`, а узкий target-интерфейс, отражающий
реальную зависимость (`ManifestCaptureTarget`/`ChildrenTarget`/`BarrierTarget`/…) — ради более строгой
interface segregation и focused-mocks.

**Вердикт v1 (синхронизировано с кодом):** не реализовано — методы facade (`Ensure*`/`Mark*`) принимают
**полный `SnapshotAdapter`**. Причина: адаптер строится один раз и является единственным domain↔SDK seam;
раздача узких ролей по сигнатурам не добавляет безопасности (тот же объект удовлетворяет всё), а только
умножает типы. Композируемые **роли facade** (`Planning`/`PlanningBarrier`/`ReadinessFault`, Р24) реализованы
и остаются точкой для focused-mocks. Capability-split по входам — future-опция: вводить отдельным изменением,
только если докажет необходимость.
...
if err := validator.Validate(adapter.SourceRef()); err != nil {  // чистый domain concern
    _ = sdk.MarkNotReady(ctx, adapter, NotReadyStatus{Reason: "InvalidSourceRef", Cause: err})  // SDK: publish (Р30)
}
```
...
### Р31. Suppression по барьеру + publish-gated иммутабельность (РЕШЕНО; заменяет core-marker suppression Р20/Р21)
**Проблема (вскрыто сверкой кода):** `CoreCaptureState{ManifestCaptured,DataCaptured}` читался SDK для
suppression, но **ни один production-контроллер эти флаги не выставлял** — это был мёртвый контракт и
источник возможного recreate-loop (после удаления MCR/VCR по TTL SDK мог бы их пересоздать).

**Решение — единый сигнал фазы планирования вместо execution-маркеров:**
- **Commit point фазы планирования = барьер** `ChildrenSnapshotReady=True` (его пишет `MarkPlanningReady`).
- **Per-artifact иммутабельность:** отдельный артефакт (child refs / MCR / VCR) иммутабелен **с момента
  публикации**, ещё до барьера. Три состояния каждого `Ensure*`:
  - **State 1** (ничего не опубликовано, барьер не закоммичен) — converge (create/reuse);
  - **State 2** (опубликовано, барьер не закоммичен) — fail-closed на расхождении desired vs published
    (`ErrTopologyDrift`/`ErrManifestDrift`/VCR-error): рестарт с недетерминированным discovery не может
    молча переписать intent;
  - **State 3** (барьер закоммичен) — SDK **inert**: `Ensure*` сразу `return nil`, ничего не создаёт/не
    переиспользует/не валидирует; владение перешло core-контроллеру.
- SDK **не зависит** от execution-фазы (не ждёт core, не читает `SnapshotContent`). Это согласует SDK
  (фаза планирования) с его же commit point.

**Удаляется:** тип `CoreCaptureState`, метод `CoreCaptureState()` на адаптере, поля
`status.manifestCaptured`/`dataCaptured` демо-CR, live-refresh маркеров и параметр `apiReader` у `New`.
**Сохраняется:** `ErrTopologyDrift`/`ErrManifestDrift`/`ReasonTopologyDrift`/`ReasonManifestDrift` и
VCR-drift — они нужны именно для State 2.
...
**Подтверждено (план совпадает с кодом; обновлено под Р31):**
- статус-поля `api/demo/v1alpha1`: `DomainCaptureState{ManifestCaptureRequestName, VolumeCaptureRequestName,
  ChildrenSnapshotRefs}` соответствует реальному `DemoVirtualDiskSnapshotStatus`/`...MachineSnapshotStatus`.
  (Исторически был и `CoreCaptureState{ManifestCaptured, DataCaptured}` core-handoff — удалён в Р31:
  suppression по барьеру, флаги `manifestCaptured`/`dataCaptured` сняты с демо-CR.)
- D4a `MergeFromWithOptimisticLock`, барьер `ConditionChildrenSnapshotReady`, MCR/VCR ensure, ownerRef-хелперы,
...
1. **Publication `Ready=False` — обобщить в `MarkNotReady` — РЕШЕНО (Р30).** Демо публикует `Ready=False`
   ...
   Узкий `MarkInvalidSource` заменён общим `MarkNotReady(NotReadyStatus{Reason,Message,Cause})`.
...
5. **`ManifestCaptureSpec` несёт полный desired-set manifest targets.** Домен решает, какие namespaced
   ...
   добавляет technical owned-PVC target из VCR при наличии захвата данных. Это важно для subtree exclude:
...
### 5.3 Публичные интерфейсы SDK v1 (синхронизировано с кодом)

> SDK v1 использует **единственный state seam — `SnapshotAdapter`**. Операции различаются не разными
> capability-target-интерфейсами, а **DTO** (`[]ChildSpec`, `VolumeCaptureSpec`, `ManifestCaptureSpec`,
> `NotReadyStatus`). Узкие per-method targets (`ManifestCaptureTarget`/`ChildrenTarget`/…) рассматривались
> для усиленной interface-segregation и focused-mocks (Р24/Р27) и **отложены** — в коде их нет (см. вердикт там).

```go
// Reason — стабильный machine-readable condition reason: домен выбирает, SDK публикует.
type Reason string

// Invariant: each snapshot node owns exactly one logical data slot (occupancy 0..1;
// manifest-only node => 0; data node => 1). Multiple independent data captures per node — unsupported in v1.

// SnapshotAdapter — ЕДИНСТВЕННЫЙ field-mapping seam между доменным CR и SDK (Р10, НЕ reconcile-time API).
// Домен реализует его один раз и передаёт во ВСЕ методы facade. (Р15.5: SDK-owned Condition-обёртка
// рассматривалась; v1 отдаёт metav1.Condition напрямую.) Suppression после барьера читается из
// GetConditions (ChildrenSnapshotReady) — отдельного core-handoff на адаптере нет (Р31, заменяет Р21).
type SnapshotAdapter interface {
    Object() client.Object                       // мост к controller-runtime
    SourceRef() SourceRef                         // spec.sourceRef (identity — домен)
    GetConditions() []metav1.Condition            // SDK мерджит по одному условию, штампует observedGeneration; барьер
    SetConditions([]metav1.Condition)
    GetDomainCaptureState() DomainCaptureState    // domain-owned: read+write
...
// Operation inputs — DTO (не capability-target-интерфейсы).
type ChildSpec          struct { Object client.Object }            // child-builder seam (Р17.5): домен строит CR
type VolumeCaptureSpec  struct { DataRef *Target }                 // ≤1 PVC; nil = manifest-only (no-op)
type ManifestCaptureSpec struct { Targets []ManifestTarget }       // полный desired-set manifest targets
// NotReadyStatus — доменно-выбранный Ready=False исход; SDK только публикует, валидацию НЕ делает (Р30).
// Requeue в DTO НЕТ: когда/как requeue решает контроллер своим ctrl.Result, не SDK.
type NotReadyStatus struct {
...
// ───────────── ПУБЛИЧНОЕ ЛИЦО SDK: interface-facade в КАНОНИЧЕСКОМ root (Р16/Р26) ─────────────
// Нейминг — на языке reconcile-intent (Ensure/Mark, Р15.7); термины домена (MCR/VCR/children) НЕ прячем;
// прячем только transport-механику (conditions/patch/retry/status-layout). ВСЕ методы принимают единый
// SnapshotAdapter (Р27-вердикт), конкретные данные операции — через DTO. Godoc lifecycle (Р18) — на CaptureSDK:
// 3 части планирования / crash-restart safety + converge-to-current-desired (Р19/Р25) / desired-set children
// publication, delete-free (Р17/Р23) / ≤1 data slot / content-free / MarkPlanningReady = barrier (не planning).
// `AlreadyCaptured` НЕ выставляется (Р20: internal TOCTOU, Ensure* идемпотентны). Source-валидация — НЕ в
// facade (Р28): identity проверяет домен сам своим SourceValidator (Р22), SDK публикует исход через MarkNotReady.
type ReadinessFault interface {
    // MarkNotReady publishes Ready=False with a domain-chosen reason. It performs NO domain validation:
...
type Planning interface {
    // EnsureChildren publishes the complete desired child set: create/adopt + publication, delete-free (Р23).
    // The SDK treats ChildSpec.Object as a domain-built template (deep-copies it, sets the parent ownerRef,
    // create-or-adopts) and derives the published SnapshotChildRef. Lifecycle (Р31, publish-gated): inert once
    // the barrier is committed (ChildrenSnapshotReady=True); before commit, a published NON-EMPTY set
    // diverging from desired (set equality on (apiVersion,kind,name), not count) is terminal ErrTopologyDrift —
    // fail-closed, creates/deletes nothing; an empty published set still converges. A duplicate desired child
    // is a non-drift error. nil/empty = leaf ("no children"), NOT "discovery failed".
    EnsureChildren(ctx context.Context, t SnapshotAdapter, desired []ChildSpec) error
    // EnsureVolumeCapture ensures the single data-ref PVC capture (≤1). nil DataRef = manifest-only = no-op
    // (delete-free: never clears a published name). Inert once the barrier is committed; before commit an
    // existing VCR whose target differs fails closed (per-artifact immutability) before adoption.
    EnsureVolumeCapture(ctx context.Context, t SnapshotAdapter, in VolumeCaptureSpec) error
    // EnsureManifestCapture ensures one MCR from the desired target set plus the owned-PVC augmentation.
    // The FINAL set must be non-empty, else ErrEmptyManifest before any cluster mutation. Inert once the
    // barrier is committed; before commit an existing MCR whose targets differ (set equality) is terminal
    // ErrManifestDrift — fail-closed, no update/patch/delete. Does not mark planning ready.
    EnsureManifestCapture(ctx context.Context, t SnapshotAdapter, in ManifestCaptureSpec) error
}

type PlanningBarrier interface {
    // MarkPlanningReady commits successful planning (barrier, NOT a planning operation): writes
    // ChildrenSnapshotReady=True/Completed; does not create MCR/VCR and does not change child refs.
    MarkPlanningReady(ctx context.Context, t SnapshotAdapter, message string) error
    // MarkPlanningFailed publishes ChildrenSnapshotReady=False with a domain-chosen reason. The reason is
    // required for drift/failure mapping (ReasonTopologyDrift/ReasonManifestDrift/ReasonCreateChildFailed/…).
    MarkPlanningFailed(ctx context.Context, t SnapshotAdapter, reason Reason, cause error) error
}

type CaptureSDK interface { ReadinessFault; Planning; PlanningBarrier }

// Sentinel-ошибки fail-closed путей (домен маппит в MarkPlanningFailed(reason, …)); все — pre-commit (State 2),
// после барьера SDK inert и drift не диагностирует:
//   ErrTopologyDrift — EnsureChildren: published NON-EMPTY child set != desired до барьера → ReasonTopologyDrift.
//   ErrManifestDrift — EnsureManifestCapture: desired targets != targets опубликованного MCR → ReasonManifestDrift.
//   ErrEmptyManifest — EnsureManifestCapture: финальный target-set пуст (домен обязан дать ≥1 target).

// New строит CaptureSDK из client (записи/кэш-чтения) + VolumeCaptureProvider. Suppression читается из
// барьерного условия адаптера, отдельный API reader не нужен (Р31).
func New(c client.Client, provider VolumeCaptureProvider) CaptureSDK

// SourceValidator — НЕ часть SDK public surface: identity-валидация остаётся доменной (Р22/Р28); домен зовёт
// её сам, SDK публикует только исход через MarkNotReady.

// VolumeCaptureProvider: ПОРТ data-capture backend'а (Р13), тип в root; реализация — internal/storagefoundation
// (NewStorageFoundationProvider). Узел связывает максимум один data-target.
type VolumeCaptureProvider interface {
...
// internal/conditions:condition-merge поверх metav1.Condition.
// internal/ownerref (Р12): UID-aware adoption, fail-closed на чужого/устаревшего родителя.
// internal/storagefoundation: VCR backend (unstructured, без Go-зависимости на foundation API).
//   Всё это зовётся ТОЛЬКО из capture intent API (root), домен сюда не импортирует.
```
Ключевое: **доменный код выражает intent через `CaptureSDK` (root)**, не зная про
`metav1.Condition`-merge/optimistic-lock (Р15: семантика читается из exported API, не из ADR).
`SnapshotAdapter` (§Р10) — **не reconcile-time API**, а одноразовый **field-mapping seam** и единственный
domain↔SDK мост; **все методы facade принимают именно его** (Р27-вердикт), а данные операции идут через DTO.
True facade (Р26): публичны только root `pkg/snapshotsdk` + `transform/`; `children`/`patch`/
...
- **Нет** поддержки нескольких независимых захватов данных на одном узле — *multiple independent data
  captures per node are explicitly unsupported* (фиксируем Вариант A: один logical data slot,
...
- Consistency-аудит (ревью 4b, устранение протечек facade):
  ...
  - Р21 (**ИСТОРИЯ; core-half отменён в Р31**): split `CaptureStatus` по ownership на `DomainCaptureState`
    (read+write, действующий) и `CoreCaptureState` (только getter) — последний удалён в Р31 (suppression по
    барьеру, не по core-маркерам).
...
- Consistency-аудит (ревью 4b, facade vs layered — устранение competing-abstraction):
  ...
  - Р27 (**РАССМОТРЕНО → ОТЛОЖЕНО в v1**): узкие per-method target-интерфейсы (`ManifestCaptureTarget`/
    `ChildrenTarget`/…) не реализованы — методы facade принимают единый `SnapshotAdapter`; capability-split — future-опция.
...
- Consistency-аудит (ревью 5, design vs фактический код — §5.1a):
  ...
  - Р30 (**РЕШЕНО**): publication `Ready=False` обобщена — узкий `MarkInvalidSource` заменён на
    `ReadinessFault.MarkNotReady(NotReadyStatus{Reason,Message,Cause})` (покрывает source +
...
- добавить публичные типы:
  ...
  - `SnapshotAdapter` (единый seam; узкие per-method targets отложены — Р27);
  - `DomainCaptureState` (Р31: `CoreCaptureState` удалён — suppression по барьеру);
  - `Reason` и sentinel-ошибки `ErrTopologyDrift`/`ErrManifestDrift`/`ErrEmptyManifest`;
  - `NotReadyStatus` (Р30: publication `Ready=False`);
  - `ChildSpec` (= `{Object client.Object}`, Р17.5: child-builder seam), `ManifestCaptureSpec`, `VolumeCaptureSpec`;
  - `VolumeCaptureProvider` (+ `NewStorageFoundationProvider`). `SourceValidator` — доменный тип (Р22/Р28), НЕ в SDK surface.
...
- constructor принимает deps: `client`, scheme/rest mapper при необходимости, `VolumeCaptureProvider`
  (Р31: `apiReader` больше не нужен — suppression по барьеру);
- все `Ensure*` (Р31): inert после коммита барьера (`ChildrenSnapshotReady=True` → `return nil`); до коммита —
  publish-gated drift (State 2);
- `EnsureChildren`: принимает полный desired-set `[]ChildSpec` (Р17.5: `ChildSpec.Object` — домен-built);
  ...
  опубликованный непустой набор != desired до барьера → `ErrTopologyDrift`; `nil`/empty = «leaf/no children»;
- `EnsureVolumeCapture`: reuse existing VCR when already published; existing VCR с другим target до барьера →
  fail-closed; enforce single data slot;
- `EnsureManifestCapture`: reuse existing MCR; existing MCR с другим target-set до барьера → `ErrManifestDrift`;
  derive manifest targets from volume targets when needed;
- `MarkPlanningReady`/`MarkPlanningFailed`: только publish outcome; не создают MCR/VCR; не меняют child refs;
- `MarkNotReady(NotReadyStatus)`: publish `Ready=False`+reason (source/artifact), без domain validation (Р30).
...
7. `snapshotsdk` (Р31): publish-gated drift — опубликованный непустой `[A,B]` до барьера, restart → desired `[A,C]`
   → `ErrTopologyDrift` (State 2, не convergence); пустой опубликованный набор (State 1) ещё сходится; после
   барьера (State 3) `Ensure*` inert.
...
9. `snapshotsdk` (Р30): `MarkNotReady` — source invalid → `Ready=False` (no requeue); artifact-missing →
   `Ready=False` (контроллер сам делает requeue через `ctrl.Result`); planning failed → `ChildrenSnapshotReady=False` (`MarkPlanningFailed`).
