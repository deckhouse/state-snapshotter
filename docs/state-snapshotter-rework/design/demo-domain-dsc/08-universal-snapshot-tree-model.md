...
Любое нарушение дерева или зависимостей, в т.ч.:
...
Удаление дочернего snapshot через GC → родитель **теряет** обязательного ребёнка в API → при следующем reconcile родитель → **`Ready=False`**, **`reason`**, например **`ChildSnapshotMissing`**.  
