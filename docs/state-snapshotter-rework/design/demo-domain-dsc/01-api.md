...
| Kind | Роль |
|------|------|
| **DemoVirtualMachineSnapshot** | Snapshot-узел VM: domain wiring в graph refs (`children*Refs`), parent/back-reference задаётся ownerRef от `NamespaceSnapshot`, `Ready` зеркалит bound content. |
...
- parent/back-reference указывается через ownerRef (`NamespaceSnapshot` или `DemoVirtualMachineSnapshot` в текущей demo-реализации); persisted graph остаётся в parent-owned `status.childrenSnapshotRefs`.
