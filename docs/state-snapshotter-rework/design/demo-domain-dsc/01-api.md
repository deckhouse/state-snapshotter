...
- **`NamespaceSnapshot`** — **текущий** root в этой архитектуре (один на сценарий «снимок namespace»); по правилам дерева он **такой же узел**, как любой `XxxxSnapshot`, а не отдельный «тип дерева» ([`08-universal-snapshot-tree-model.md`](08-universal-snapshot-tree-model.md)). Под root **в этом треке** **нет** вложенного **`NamespaceSnapshot`** (**INV-T1** в [`05-tree-and-graph-invariants.md`](05-tree-and-graph-invariants.md)) — **не** потому что kind «особый», а потому что дети задаются как **heterogeneous** `XxxxSnapshot`; «верхность» — отсутствие в продукте snapshot **выше** namespace, а не kube-роль «доменного контроллера». Концептуально root **не** «владеет» доменными CRD как отдельной сущностью — он **только** верхний узел **текущего** run; ниже по дереву могут быть **любые** snapshot kinds, встроенные в **общую** модель refs + **`Ready`** ([`05`](05-tree-and-graph-invariants.md)).
