...
**Common content contract:** the common controller owns `SnapshotContent` lifecycle, ObjectKeeper/Retain, MCP refs, child content refs, and aggregation over `SnapshotContent.status.childrenSnapshotContentRefs`. It is an aggregator/lifecycle controller, not a domain planner/executor: MCR/VCR/DataExport/VolumeSnapshot request creation stays in snapshot/domain controllers. Domain modules register source resource and snapshot CRD through DSC; content GVK is fixed to `storage.deckhouse.io/v1alpha1, Kind=SnapshotContent`.

**Not owned by a domain controller:** `SnapshotContent.status`, DSC `RBACReady`, RBAC creation, and parent status. Content lifecycle is intentionally separate from namespaced snapshot lifecycle in the current Retain/ObjectKeeper model: each snapshot controller owns only its own snapshot status and direct snapshot child graph refs; `SnapshotContentController` owns content status/MCP links and child content refs.
