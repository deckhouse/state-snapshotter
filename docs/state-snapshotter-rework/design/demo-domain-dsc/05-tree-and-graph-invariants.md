...
**Контракт записи refs (владение на родителе).** Запись в **`children*Refs`** **аддитивная** (**merge** по ключу элемента ref / согласованному контракту PR5 в spec): контроллер **не** удаляет и **не** подменяет целиком чужие элементы списка — только **свои** (те дочерние узлы, за создание которых он отвечает). **Удаление** ссылки с родителя — **владелец** этой записи (тот же контроллер при снятии дочернего узла) или явная политика родителя на reconcile; конкурирующие **полные** замены `status` со списком refs без merge — **вне** контракта (см. [`03-snapshot-flow.md`](03-snapshot-flow.md), [`02-dsc-wiring.md`](02-dsc-wiring.md)).
...
| # | Обязанность |
|---|-------------|
| D1 | Создают demo snapshot/content, VS, MCP; заполняют **`childrenSnapshotRefs`** / **`childrenSnapshotContentRefs`** на своих и родительских узлах по правилам §2 и **контракту владения** записью refs (§1, абзац «Контракт записи refs»); соблюдают **согласованность refs ↔ API** (§1, абзац под таблицей «Как появляются дети»). |
