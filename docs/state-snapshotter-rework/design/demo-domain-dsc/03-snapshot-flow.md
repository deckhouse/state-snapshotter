...
- инициирует каскад через **`ownerReference`** и **финализаторы** для объектов, входящих в **текущий snapshot-run**;
...
- модель **не** предполагает дочерние **`NamespaceSnapshot`**, поэтому **нет** специальной логики их каскадного удаления — дочерние узлы PR5 это **heterogeneous** kinds, снимаемые вместе с run по **ownerRef**/политике модуля.
