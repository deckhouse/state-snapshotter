...
- После формулы **Accepted + AccessGranted** (+ поколения) CSD участвует в **merge** с bootstrap; `pkg/unifiedruntime` делает **additive** watches без рестарта pod для новых eligible типов.
