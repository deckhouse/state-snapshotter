...
- Типовые ресурсы модуля: **Snapshot**, **SnapshotContent**, **NamespaceSnapshot** и т.д. — то, что перечислено в **bootstrap** списке desired пар GVK (`pkg/unifiedbootstrap`), пока DSC не переопределяет набор для других API.
