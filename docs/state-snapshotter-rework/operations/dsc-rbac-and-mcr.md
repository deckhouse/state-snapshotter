...
**Назначение:** модуль (через свой hook/оператор) создаёт кластерный объект DSC и указывает **маппинг** имён CRD: какой source resource обслуживает какой `*Snapshot` тип. В v0 DSC mapping — это `resourceCRDName -> snapshotCRDName`; content side всегда общий cluster-scoped `storage.deckhouse.io/SnapshotContent`.
...
- DSC **не генерирует** RBAC в текущей реализации; он лишь **ожидает**, что модуль выставит **`RBACReady=True`**, когда политика применена и effective permissions уже существуют.
