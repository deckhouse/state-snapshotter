...
| N5 data-layer (`dataRefs[]`, logical `SnapshotContent`) | ⬜ overall; **PR-1** ✅; **PR-2** ✅; **PR-3** ✅; **PR-4** blocked on **PR-F** (storage-foundation) — **§2.4.5** |
| M* | ⬜ по плану |
...
**N5 data-layer:** **`dataRefs[]`** on logical **`SnapshotContent`**. **PR-1–PR-3** ✅ (API, SCC readiness, restore tree). **PR-4** (state-snapshotter VCR publish) **blocked on PR-F** bulk VCR in storage-foundation (`docs/pr-f-bulk-volumecapturerequest.md`). **§2.4.5** in [`design/implementation-plan.md`](../design/implementation-plan.md).
