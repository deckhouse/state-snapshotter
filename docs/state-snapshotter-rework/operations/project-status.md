...
**Текущий фокус:** **N1** по namespace-flow **формально закрыт** (см. [`design/implementation-plan.md`](../design/implementation-plan.md) §2.4 / §2.4.1). **N2a** (один root, manifests-only, MCR→MCP, download одного снимка, негативные integration: delete during capture, recreate root, CapturePlanDrift) — в коде и тестах в состоянии, близком к merge/review; ограничения (root OK без TTL-retention, list без pagination) задокументированы в плане и design. **Следующий этап — N2b:** декомпозиция **короткими PR** — **[§2.4.2](../design/implementation-plan.md)** (вход с **PR1**: только поля графа `childrenSnapshotRefs` / `childrenSnapshotContentRefs` + docs/spec, без orchestration и без смены семантики Ready N2a-leaf). Продуктовое ТЗ — [`snapshot-rework/`](../../../snapshot-rework/). **M1/M2 (manifest)** — по gate в плане §4. Обзор NS — [`design/namespace-snapshot-controller.md`](../design/namespace-snapshot-controller.md).
