...
| N5 data-layer (`dataRefs[]`, logical `SnapshotContent`) | ⬜ overall; **PR-1** ✅; **PR-2** ✅; **PR-3** ✅; **PR-F** ✅; **PR-4** ✅; **PR-5** ✅; **PR-6** ✅; **PR-7** ✅; **PR-8** ✅ `hack/pr8-smoke.sh` cluster (local-thin) — **§2.4.5** |
| M* | ⬜ по плану |
...
**N5 data-layer:** **`dataRefs[]`** on logical **`SnapshotContent`**. **PR-1–PR-3** ✅ (API, SCC readiness, restore tree). **PR-F** ✅ bulk VCR in storage-foundation. **PR-4** ✅ one VCR per content → publish `dataRefs[]`. **PR-5/PR-6** ✅ production PVC ownership (root residual vs domain node; stub test-only). **PR-7** ✅ envtest vertical slice (`n5_pr7_*_integration_test.go`). **PR-8** ✅ cluster e2e [`hack/pr8-smoke.sh`](../../../hack/pr8-smoke.sh) (local-thin, two-PVC subtree, real bulk VCR→VSC); requires module deploy + admin kubeconfig RBAC on `snapshots.storage.deckhouse.io`. **§2.4.5** in [`design/implementation-plan.md`](../design/implementation-plan.md).
