...
**Always-on runtime:** `NamespaceSnapshot`, common `SnapshotContent`, DSC reconciler, graph registry, generic `SnapshotController` / `unifiedruntime.Syncer`, and dynamic DSC hot-add path initialize on the single v0 runtime path.
