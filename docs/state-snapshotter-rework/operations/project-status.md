...
| N5 data-layer (`dataRefs[]`, logical `SnapshotContent`) | ⬜ overall; **PR-1** ✅ (`SnapshotContent.dataRefs[]` in CRD + mechanical consumers); **PR-F** (foundation bulk VCR) → **PR-2** SCC readiness → **PR-4** publish — **§2.4.5** |
| M* | ⬜ по плану |
...
**N5 data-layer:** target model corrected to **`dataRefs[]`** on logical **`SnapshotContent`** (not one-volume-per-content, not synthetic PVC child nodes). Docs roadmap **PR-0…PR-9** — [`design/implementation-plan.md`](../design/implementation-plan.md) **§2.4.5**. **PR-1** landed in code (`SnapshotContent.dataRefs[]`). **PR-F** (bulk VCR in storage-foundation) is prerequisite for data capture; next state-snapshotter steps **PR-2** SCC, then **PR-4** one-VCR publish path.
