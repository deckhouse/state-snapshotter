...
**Temporary `snapshotRef` dependency:** current runtime still uses `SnapshotContent.spec.snapshotRef` to finish content aggregation from the live snapshot. This is tracked as debt before the future retained/self-contained content refactor; do not describe current `SnapshotContent` as fully independent from `Snapshot`.
