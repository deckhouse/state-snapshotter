...
**N2b — generic [`spec/system-spec.md`](../spec/system-spec.md) §3 в коде:** **E1–E5** — как в таблице и [`design/implementation-plan.md`](../design/implementation-plan.md) **§3-E1–E5**. **E6:** parent **`Ready`** по строгим refs + **`Get`** по GVK; registry только для **E5** / graph walk при непустых **`childrenSnapshotRefs`** на exclude-пути. Дерево run **namespace-local** (**§3.2** spec): child→parent relay **без** cluster-wide list; cross-namespace **`childrenSnapshotRefs[].namespace`** — fail-closed. Пустые **`childrenSnapshotRefs`** — полный namespace **list для manifest capture** (не смешивать с графом E6). Кластерный smoke registry — **`hack/snapshot-graph-registry-smoke.sh`**.
