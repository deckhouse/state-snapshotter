...
**Текущий фокус:** **N1** по namespace-flow **формально закрыт** (см. [`design/implementation-plan.md`](../design/implementation-plan.md) §2.4 / §2.4.1). **N2a** (один root, manifests-only, MCR→MCP, download одного снимка, негативные integration: delete during capture, recreate root, CapturePlanDrift) — в коде и тестах в состоянии, близком к merge/review; ограничения (root OK без TTL-retention, list без pagination) задокументированы в плане и design. **N2b PR1** (форма графа в API/CRD: `childrenSnapshotRefs` / `childrenSnapshotContentRefs`, без orchestration) — в коде; **следующий шаг — N2b PR2+** по **[§2.4.2](../design/implementation-plan.md)** (wiring дерева, ожидание child **Ready**, затем aggregated download). Продуктовое ТЗ — [`snapshot-rework/`](../../../snapshot-rework/). **M1/M2 (manifest)** — по gate в плане §4. Обзор NS — [`design/namespace-snapshot-controller.md`](../design/namespace-snapshot-controller.md).
