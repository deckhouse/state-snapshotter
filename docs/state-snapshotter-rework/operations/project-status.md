...
| N5 data-layer (`dataRefs[]`, logical `SnapshotContent`) | ⬜ overall; **PR-1** ✅; **PR-2** ✅; **PR-3** ✅; **PR-F** ✅; **PR-4** ✅; **PR-5** ✅; **PR-6** ✅; **PR-7** ✅ — **§2.4.5** |
| M* | ⬜ по плану |
...
**N5 data-layer:** **`dataRefs[]`** on logical **`SnapshotContent`**. **PR-1–PR-3** ✅ (API, SCC readiness, restore tree). **PR-F** ✅ bulk VCR in storage-foundation. **PR-4** ✅ one VCR per content → publish `dataRefs[]`. **PR-5** ✅ scoped PVC in MCR targets + explicit-target MCP filter. **PR-6** ✅ subtree `pvcUID` coverage resolver (residual namespace PVCs; duplicate fail-closed). **PR-7** ✅ envtest two-PVC subtree vertical slice (`n5_pr7_*_integration_test.go`). **§2.4.5** in [`design/implementation-plan.md`](../design/implementation-plan.md).
