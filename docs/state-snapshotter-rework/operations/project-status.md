...
| N5 data-layer (`dataRefs[]`, logical `SnapshotContent`) | ⬜ overall; **PR-1** ✅; **PR-2** ✅; **PR-3** in progress (restore tree); **PR-F** → **PR-4** — **§2.4.5** |
| M* | ⬜ по плану |
...
**N5 data-layer:** **`dataRefs[]`** on logical **`SnapshotContent`**. **PR-1** ✅, **PR-2** ✅. **PR-3** in progress: restore tree carries per-content `dataRefs[]`, PVC→VRR fail-closed. **PR-F** / **PR-4** publish unchanged. **§2.4.5** in [`design/implementation-plan.md`](../design/implementation-plan.md).
