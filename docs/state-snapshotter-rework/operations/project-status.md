...
**Текущий фокус:** **N1** по namespace-flow **формально закрыт** (см. [`design/implementation-plan.md`](../design/implementation-plan.md) §2.4). **N2** — **первый real capture** по пошаговому плану **[§2.4.1](../design/implementation-plan.md)** (профиль GVR → артефакт v1 → контракт runner → Job orchestration → runner → `NamespaceSnapshotContent.status` → **ObjectKeeper** / retention → integration). Продуктовое ТЗ — [`snapshot-rework/`](../../../snapshot-rework/) (из `docs/` ТЗ не переписываем). **M1/M2 (manifest)** — после закрытого **N2** или явного gate в плане. Опционально — T5, T9, …. **ValidatingWebhook** — не в ближайшем плане. Обзор NS — [`design/namespace-snapshot-controller.md`](../design/namespace-snapshot-controller.md). При смене контракта обновляй [`README.md`](../README.md), [`runbook-degraded-and-unified-runtime.md`](runbook-degraded-and-unified-runtime.md), [`dsc-rbac-and-mcr.md`](dsc-rbac-and-mcr.md).
