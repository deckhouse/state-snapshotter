...
| N5 data-layer (`dataRefs[]`, logical `SnapshotContent`) | ⬜ overall; **PR-1** ✅; **PR-2** ✅ (`resolveDataReadiness`); next **PR-3** / **PR-F** → **PR-4** — **§2.4.5** |
| M* | ⬜ по плану |
...
**N5 data-layer:** **`dataRefs[]`** on logical **`SnapshotContent`**. **PR-1** ✅, **PR-2** ✅ (`resolveDataReadiness` for common content). Next: **PR-3** restore tree, **PR-F** bulk VCR, **PR-4** publish. **§2.4.5** in [`design/implementation-plan.md`](../design/implementation-plan.md).
