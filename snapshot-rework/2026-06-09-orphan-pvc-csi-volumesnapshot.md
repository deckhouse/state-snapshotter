...
> **Ready-gate addendum (conditions-model ADR `2026-06-03-snapshot-conditions-model.md` §2.3, 2026-06-28).** ~~Финальная residual/orphan-PVC-волна **явно гейтит первый `Ready=True`** корневого `SnapshotContent` через латч-поле `SnapshotContent.status.residualVolumeCapture.phase=Complete` и ногу `Ready=False/ResidualVolumeCapturePending`.~~ **SUPERSEDED (2026-07-09, wave7 `80647b9`):** латч `residualVolumeCapture` и reason `ResidualVolumeCapturePending` удалены; их fail-closed-роль поглощена гейтом `ChildrenReady` / reason `ChildrenLinkPending` (набор детей полон к `phase=Planned`, списки append-only, без отдельного барьера завершения волны). См. INV-ORPHAN6 ниже.
...
- **INV-ORPHAN6 (Ready-gate, 2026-06-28) — SUPERSEDED (2026-07-09, wave7 `80647b9`):** the fail-closed
  "first `Ready=True`" barrier no longer uses a dedicated `status.residualVolumeCapture.phase=Complete`
  latch (removed). The invariant it protected — the root MUST NOT emit its first `Ready=True` while a
  declared orphan child is not yet linked/ready — is now enforced by the `ChildrenReady` gate (reason
  `ChildrenLinkPending`): the declared child set is complete at `phase=Planned` and the ref lists are
  append-only, so a late-linking orphan node keeps `ChildrenReady=False` until its edge is written. See
  conditions-model ADR §2.3 (also superseded).
