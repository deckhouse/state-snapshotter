...
- **Зависимость (удовлетворена; не изменение):** orphan-PVC restore (PVC → `dataSourceRef` на VS) опирается на capture-поведение orphan-PVC (VS как `childrenSnapshotRefs` visibility-leaf + VSC в root `dataRefs[]` с `deletionPolicy=Retain`). Это **уже реализовано** в коде (`internal/controllers/snapshot/orphan_pvc_volume_snapshot.go`: `reconcileOrphanPVCVolumeSnapshotChildLeaves`, `PublishSnapshotContentDataRefs`, `ensureVolumeSnapshotContentRetain`; spec §3.9.11) и покрыто кластерным e2e (`hack/snapshot-tree-demo-e2e.sh`: стадии `orphan-pvc-vs`, `orphan-pvc-cleanup`, `referenced-pvc-without-disk-csd`). Этот ADR capture не реализует и не меняет — только читает уже публикуемые refs. **Зависимость orphan-PVC capture удовлетворена текущим кодом.** Если при реализации обнаружится, что в конкретной тест-фикстуре отсутствует требуемый VS visibility-leaf / VSC `dataRef`, — это баг фикстуры/setup, а **не** повод добавлять новые поля или менять capture-логику.
...
Cluster smoke — при необходимости в рамках `hack/demo-e2e.sh` / `hack/snapshot-tree-demo-e2e.sh`
