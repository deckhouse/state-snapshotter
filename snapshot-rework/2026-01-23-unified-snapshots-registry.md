...
> **Модуль `state-snapshotter`:** нормативные выдержки и статусы реализации — [`docs/internal/state-snapshotter-rework/spec/system-spec.md`](../docs/internal/state-snapshotter-rework/spec/system-spec.md), [`design/implementation-plan.md`](../docs/internal/state-snapshotter-rework/design/implementation-plan.md).
> **Transition note:** examples below that mention `resourceCRDName` / `snapshotCRDName` are historical draft material. The current implementable CSD contract is GVK-only (`source.apiVersion/source.kind`, `snapshot.apiVersion/snapshot.kind`, `priority`) and is defined in `docs/internal/state-snapshotter-rework/spec/system-spec.md`.
...
- либо **ограниченный** список GVK за **feature flag** (временная замена статического bootstrap в `main.go` до DSC).
...
**Владение модулем (без отдельного поля в spec).** Ранее в v1alpha1 существовало поле `spec.ownerModule` как декларативный якорь владения модулем; оно **удалено из контракта**, поскольку контроллер state‑snapshotter его не читал, а admission‑enforcement привязки `ownerModule`↔SA так и не был реализован (см. историю изменений). Привязка регистрации к модулю и выдача domain‑RBAC остаются ответственностью внешнего Deckhouse RBAC‑controller/hook и сигнализируются через `status.conditions[RBACReady=True]`; кто именно владеет DSC, определяется RBAC на сам ресурс DSC (кто имеет право его создавать/менять), а не полем внутри `spec`.

**ValidatingWebhook:** только **reject** невалидных изменений `spec`; **не пишет** `status` DSC. При недоступности webhook поведение — стандартное для кластера (fail‑open vs fail‑closed задаётся политикой apiserver/webhook).
