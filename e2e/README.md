...
- `E2E_CAPTURE_STALL`: **opt-in** (green as a suite spec on a dev cluster, 2026-08-07) — the
  case-B stall-diagnostics spec: a data leg whose CSI driver runs
  ...
  this spec ends before any CSI RPC anyway. Exact command:
  ...
  Teardown: deleting the root `Snapshot` only detaches whatever the tree published — it is then
  retained for `snapshotTtlAfterDelete` (720h by default) as a recycle bin, so the spec deletes the
  root `Snapshot` first and then the root `ObjectKeeper` (break-glass annotation + delete, as
  `volumedata_gc_test.go` does) and lets the ownerRef chain cascade through the contents. The order
  matters: the keeper follows the live `Snapshot`, so deleting it first reclaims the tree but the
  controller recreates the keeper at once, leaving it to count down the TTL afterwards.

  One piece of residue is inherent to the environment and is left alone on purpose: the produced
  `VolumeSnapshotContent` stays `Terminating`, because it holds the snapshot-controller's
  bound-protection finalizer and no sidecar can complete `DeleteSnapshot`. The handle is synthetic,
  so no backend snapshot exists and clearing it by hand leaks nothing — but it is a foreign
  finalizer, so the spec does not strip it:
