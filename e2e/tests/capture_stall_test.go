...
//
// The volume is provisioned STATICALLY on purpose (see buildCaptureStallSource).
//
// Teardown: this spec deletes the root Snapshot and then the root ObjectKeeper (see the DeferCleanup in
// the spec body), in that order. Deleting the Snapshot only detaches whatever the tree published — the
// keeper then RETAINS it for snapshotTtlAfterDelete (720h by default) as a recycle bin, which is by
// design and not something a fixture can wait out. The reverse order does not work: a keeper deleted
// while its Snapshot is still alive is recreated at once.
//
// One piece of residue is inherent to the environment under test and is deliberately left alone: the
// VolumeSnapshotContent stays Terminating, because it carries the cluster snapshot-controller's
// bound-protection finalizer and there is no sidecar to complete DeleteSnapshot. The volumeHandle is
// synthetic, so no backend snapshot ever existed and clearing that finalizer by hand leaks nothing — but
// it is a foreign finalizer, so the spec does not touch it.
const (
...
// csDropSnapshotControllerFinalizers clears the finalizers of a VolumeSnapshotContent and returns the
// object the API server accepted, i.e. proof that the "no finalizer" state really existed and was
// published to every watcher.
//
// This is deliberate fault injection, not cleanup. The regression under guard needs the common
// snapshot-controller's finalizer to be ABSENT while the content is young; on a healthy cluster the
// finalizer lands a few seconds after creation, so waiting for that window to be observed naturally is a
// race the spec would usually lose. Removing it recreates the exact input, and because storage-foundation
// watches VolumeSnapshotContent, this very write wakes the classifier — a regression answers in seconds.
//
// It leaks nothing: the finalizer only guards deletion, nothing is deleting the content here, and the
// snapshot-controller puts it back on its next sync.
func csDropSnapshotControllerFinalizers(ctx context.Context, name string) (*unstructured.Unstructured, error) {
	patched, err := suiteDyn.Resource(volumeSnapshotContentGVR).Patch(
		ctx, name, types.MergePatchType, []byte(`{"metadata":{"finalizers":[]}}`), metav1.PatchOptions{},
	)
	if err != nil {
		return nil, fmt.Errorf("clear finalizers of VolumeSnapshotContent %s: %w", name, err)
	}
	return patched, nil
}

// csStackUnavailableEvents counts Warning events that blame the snapshot stack on one VolumeCaptureRequest.
// The condition can be overwritten by a later reconcile, but an event cannot be taken back, so this is the
// stricter half of the negative assertion.
func csStackUnavailableEvents(ctx context.Context, ns, vcrName string) (int, error) {
	list, err := suiteClientset.CoreV1().Events(ns).List(ctx, metav1.ListOptions{
		FieldSelector: "involvedObject.name=" + vcrName + ",reason=" + csReasonStackUnavailable,
	})
	if err != nil {
		return 0, fmt.Errorf("list %s events for %s/%s: %w", csReasonStackUnavailable, ns, vcrName, err)
	}
...
//  2. NOTHING is diagnosed while the content is younger than the no-pickup grace, asserted on the exact
//     input that once produced a false alarm: a young content with the snapshot-controller finalizer
//     removed on purpose (neither a Stalled condition nor a SnapshotStackUnavailable event may appear);
...
func captureStallSpecs() {
	Context("Capture stall diagnostics (no snapshotter sidecar -> SnapshotExecutionUnobservable)", func() {
		...
		It("diagnoses the unobservable data leg without failing it or collecting it", func() {
			...
			// Whatever the tree publishes outlives the root Snapshot: the contents are cluster-scoped and are
			// owned by a FollowObjectWithTTL ObjectKeeper that retains them for snapshotTtlAfterDelete (720h
			// by default) as a recycle bin. So teardown takes the route volumedata_gc_test.go takes — delete
			// the keeper and let the ownerRef chain cascade — but the ORDER matters. The keeper exists from
			// the moment the tree is created and follows the LIVE root Snapshot, so deleting it first does
			// reclaim the tree, and then the controller immediately recreates the keeper; the namespace
			// deletion registered in BeforeAll (which Ginkgo runs after this cleanup, container-level
			// cleanups run last) then leaves that fresh keeper counting down the TTL. Deleting the root
			// Snapshot here removes what recreates it: the keeper flips to Expiring and stays deleted.
			rootSnap, err := getResource(ctx, snapshotGVR, srcNS, csRootSnapshotName)
			Expect(err).NotTo(HaveOccurred())
			okName := names.ObjectKeeperName(rootSnap.GetUID())
			DeferCleanup(func() {
				if cleanupSkipped() {
					GinkgoWriter.Printf("%s: keeping root Snapshot %s and ObjectKeeper %s\n", keepReason(), csRootSnapshotName, okName)
					return
				}
				cctx, ccancel := context.WithTimeout(context.Background(), 10*time.Minute)
				defer ccancel()
				if derr := deleteWithAllowDelete(cctx, snapshotGVR, srcNS, csRootSnapshotName); derr != nil {
					GinkgoWriter.Printf("  cleanup: delete root Snapshot %s/%s: %v\n", srcNS, csRootSnapshotName, derr)
					return
				}
				// A keeper that never surfaces retains nothing, so its absence is not residue — hence the
				// bounded wait reports and gives up instead of failing the spec.
				deadline := time.Now().Add(2 * time.Minute)
				for {
					_, gerr := getResource(cctx, objectKeeperGVR, "", okName)
					if gerr == nil {
						break
					}
					if !apierrors.IsNotFound(gerr) {
						GinkgoWriter.Printf("  cleanup: read root ObjectKeeper %s: %v\n", okName, gerr)
						return
					}
					if time.Now().After(deadline) {
						GinkgoWriter.Printf("  cleanup: root ObjectKeeper %s never appeared, so it retains nothing\n", okName)
						return
					}
					time.Sleep(pollInterval)
				}
				if derr := deleteWithAllowDelete(cctx, objectKeeperGVR, "", okName); derr != nil {
					GinkgoWriter.Printf("  cleanup: delete root ObjectKeeper %s: %v\n", okName, derr)
				}
			})
			...
			// content appears — before the cluster snapshot-controller can add its finalizer. Blaming the
			// snapshot stack there accuses a healthy stack on every capture, and asserting only the end
			// state would let that regression back in unnoticed: a premature diagnosis is later overwritten
			// by the correct one and leaves the end state intact.
			//
			// The guarded input is "young content AND no finalizer", so the spec produces it instead of
			// hoping to catch it: csDropSnapshotControllerFinalizers removes the finalizer and the returned
			// object proves the state existed. Waiting is then pointless — the removal itself is the watch
			// event that drives the classification.
			negativeWindow := csNoPickupGrace - csGraceMargin - time.Since(content.GetCreationTimestamp().Time)
			Expect(negativeWindow).To(BeNumerically(">", 0),
				"the content was already %s old when discovered, leaving no room inside the %s grace to assert the negative case; "+
					"this spec cannot certify the regression guard under such a delay",
				time.Since(content.GetCreationTimestamp().Time).Truncate(time.Second), csNoPickupGrace)

			By("Removing the snapshot-controller finalizer to reproduce the false-alarm input on a young content")
			stripped, err := csDropSnapshotControllerFinalizers(ctx, content.GetName())
			Expect(err).NotTo(HaveOccurred())
			Expect(stripped.GetFinalizers()).To(BeEmpty(),
				"the API server must have accepted the finalizer-less state, otherwise the guarded input was never produced")
			Expect(time.Since(stripped.GetCreationTimestamp().Time)).To(BeNumerically("<", csNoPickupGrace),
				"the injection only reproduces the regression while the content is younger than the grace")
			GinkgoWriter.Printf("  injected: content %s has no finalizer at age %s (grace %s)\n",
				stripped.GetName(), time.Since(stripped.GetCreationTimestamp().Time).Truncate(time.Second), csNoPickupGrace)
			...
			Consistently(func(g Gomega) {
				...
				g.Expect(reason).NotTo(Equal(csReasonStackUnavailable),
					"a young content must never be blamed on the snapshot stack: the finalizer is simply not there yet")
				if ok {
					g.Expect(st).NotTo(Equal("True"), "no stall may be diagnosed within the no-pickup grace, got reason %q", reason)
				}

				// An event cannot be retracted, so it catches a premature diagnosis even if the condition
				// that carried it was already corrected between two polls.
				events, gerr := csStackUnavailableEvents(ctx, srcNS, vcr.GetName())
				g.Expect(gerr).NotTo(HaveOccurred())
				g.Expect(events).To(BeZero(), "no %s event may be emitted inside the no-pickup grace", csReasonStackUnavailable)
			...
			// Informational: the snapshot-controller is expected to have put its finalizer back by now, and
			// that arrival must not have produced a diagnosis either (asserted above).
