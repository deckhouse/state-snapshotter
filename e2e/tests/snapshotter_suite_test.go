...
var _ = Describe("state-snapshotter e2e", Ordered, ContinueOnFailure, func() {
	...
	captureStallSpecs()              // capture_stall_test.go: data leg on a snapshotter-less CSI driver -> VCR Stalled=True/SnapshotExecutionUnobservable, DataCaptureStalled upward, Ready stays TargetsPending, request not collected (opt-in E2E_CAPTURE_STALL)
